...
Para criar deliberadamente os dois acessos de homologação, execute `./scripts/provision-homologation.sh`. Para operar apenas a Santa Clara sem alterar uma senha já persistida, use `dotnet run --project src/Hosts/Agro360.Migrator -- provision-santa-clara --environment Homologation`. A redefinição é uma operação separada: leia a nova senha sem eco (`read -r -s AGRO360_PROVISION_SANTA_CLARA_PASSWORD; export AGRO360_PROVISION_SANTA_CLARA_PASSWORD`) e execute `dotnet run --project src/Hosts/Agro360.Migrator -- reset-santa-clara-password --environment Homologation`; ao final, `unset AGRO360_PROVISION_SANTA_CLARA_PASSWORD`. Esses comandos são proibidos em Production, validam tenant e identidade, persistem em transação e verificam no PostgreSQL o hash recém-gravado. A redefinição revoga somente as sessões de `santa-clara` / `admin@santaclara.agro360.local`. Não existe senha padrão recuperável.

O SuperAdmin exige ainda `AGRO360_DATA_PROTECTION_KEYS_PATH`, um segredo Base32 em `AGRO360_PROVISION_SUPERADMIN_TOTP_SECRET` e a confirmação de um código atual em `AGRO360_PROVISION_SUPERADMIN_TOTP_CODE`. API e Migrator devem compartilhar o mesmo diretório persistente de chaves (`DataProtection__KeysPath` na API) para que o segredo protegido seja legível. A API nunca provisiona contas na inicialização e não consulta senha de usuário na configuração: depois do comando opt-in do Migrator, identidade, hash, perfis, autoridade global e MFA são lidos exclusivamente do PostgreSQL.
