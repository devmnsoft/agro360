...
create table if not exists agro360.finance_commercial_receivables (
    id uuid primary key,
    tenant_id uuid not null references agro360.tenancy_tenants(id),
    farm_id uuid not null,
    sale_id uuid null,
...
create index if not exists ix_commercial_receivables_due
    on agro360.finance_commercial_receivables (tenant_id, status, due_date);
...
select agro360.platform_enable_tenant_rls('agro360.finance_commercial_receivables');
...
drop index if exists agro360.ix_outbox_pending;
create index if not exists ix_outbox_pending
    on agro360.platform_outbox_messages (tenant_id, next_attempt_at, occurred_at)
...
create or replace function agro360.platform_enable_tenant_rls(target_table regclass) returns void language plpgsql as $$ begin execute format('alter table %s enable row level security',target_table); execute format('alter table %s force row level security',target_table); execute format('drop policy if exists tenant_isolation on %s',target_table); execute format('create policy tenant_isolation on %s using (tenant_id=agro360.platform_current_tenant_id()) with check (tenant_id=agro360.platform_current_tenant_id())',target_table); end $$;
...
-- Mantém agro360.platform_enable_tenant_rls disponível para seções posteriores
-- (compras 6.4.1, pecuária 6.8.0, frota 6.9.0) do instalador consolidado.
...
begin
    foreach table_name in array array[
        'finance_chart_of_accounts', 'finance_cost_centers', 'finance_payables',
        'finance_receivables', 'finance_settlements', 'finance_manual_entries',
        'finance_cost_allocations'
    ]
    loop
        execute format('alter table agro360.%I enable row level security', table_name);
        execute format('alter table agro360.%I force row level security', table_name);
        execute format('drop policy if exists tenant_isolation on agro360.%I', table_name);
        execute format(
            'create policy tenant_isolation on agro360.%I using (tenant_id=agro360.platform_current_tenant_id()) with check (tenant_id=agro360.platform_current_tenant_id())',
            table_name);
    end loop;
end $$;
...
create table if not exists agro360.intelligence_legacy_alert_rules(
...
create table if not exists agro360.intelligence_legacy_alerts(
 id uuid primary key, tenant_id uuid not null references agro360.tenancy_tenants(id), farm_id uuid, rule_id uuid references agro360.intelligence_legacy_alert_rules(id),
...
create unique index if not exists ux_intelligence_legacy_alert_dedup on agro360.intelligence_legacy_alerts(tenant_id,fingerprint) where status in('OPEN','SNOOZED');
create index if not exists ix_intelligence_legacy_alert_queue on agro360.intelligence_legacy_alerts(tenant_id,status,severity,detected_at desc);
create table if not exists agro360.intelligence_legacy_alert_audit(
 id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),alert_id uuid not null references agro360.intelligence_legacy_alerts(id),
...
insert into agro360.intelligence_legacy_alert_rules(id,tenant_id,type,name,severity,parameters,created_by)
...
insert into agro360.identity_permissions(code,module,description) values ('compliance.read','Compliance','Consultar compliance, auditoria e exportação.'),('compliance.write','Compliance','Gerenciar compliance e evidências.'),('compliance.approve','Compliance','Aprovar, bloquear e encerrar controles.'),('esg.read','ESG','Consultar indicadores ESG e carbono.'),('esg.write','ESG','Registrar indicadores ESG e inventário de carbono.') on conflict(code) do update set module=excluded.module,description=excluded.description;
...
language sql security definer stable set search_path=pg_catalog,agro360 as $$
 ...
 encode(public.digest(d.certificate_code||d.id::text,'sha256'),'hex')
...
create table if not exists agro360.integrations_iot_readings(id uuid primary key,tenant_id uuid not null,device_id uuid not null,sensor_type varchar(40) not null,value numeric(18,6) not null,unit varchar(20) not null,recorded_at timestamptz not null,latitude numeric(10,7),longitude numeric(10,7),critical boolean not null default false,unique(tenant_id,id),foreign key(tenant_id,device_id) references agro360.integrations_iot_devices(tenant_id,id));
...
insert into agro360.identity_permissions(code,module,description) values('cooperative.read','Cooperativas','Consultar rede cooperativa e portal.'),('cooperative.write','Cooperativas','Gerenciar cooperados, assistência e negócios.'),('cooperative.approve','Cooperativas','Aprovar compras, contratos e repasses.') on conflict(code) do update set module=excluded.module,description=excluded.description;
...
create table if not exists agro360.fiscal_issuance_documents(id uuid primary key default gen_random_uuid(),tenant_id uuid not null references agro360.tenancy_tenants(id),document_type varchar(30) not null check(document_type in('NFE','NFCE','NFSE','CTE','MDFE','PRODUCER_INVOICE','FISCAL_WAYBILL','TRANSPORT_DOCUMENT','EXPORT_DOCUMENT','OTHER')),access_key varchar(44),number varchar(30),series varchar(10),issuer_document varchar(20) not null,recipient_document varchar(20) not null,total_amount numeric(18,2) not null check(total_amount>=0),status varchar(30) not null default 'DRAFT' check(status in('DRAFT','IMPORTED','INTERNALLY_VALIDATED','PENDING_SUBMISSION','SENT_TO_PROVIDER','AUTHORIZED','REJECTED','CANCELLED','DENIED','VOIDED','INTEGRATION_ERROR')),provider_protocol varchar(100),provider_response_hash char(64),rejection_reason varchar(1000),cancellation_reason varchar(1000),order_id uuid,contract_id uuid,lot_id uuid,delivery_id uuid,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,deleted_at timestamptz,unique(tenant_id,id));
create unique index if not exists ux_fiscal_issuance_access_key on agro360.fiscal_issuance_documents(tenant_id,access_key) where access_key is not null;
create table if not exists agro360.fiscal_issuance_document_items(id uuid primary key default gen_random_uuid(),tenant_id uuid not null,document_id uuid not null,line_number int not null check(line_number>0),description varchar(300) not null,ncm varchar(8),cfop varchar(4),unit varchar(10) not null,quantity numeric(18,4) not null check(quantity>0),unit_value numeric(18,6) not null check(unit_value>=0),total_value numeric(18,2) not null check(total_value>=0),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,unique(tenant_id,document_id,line_number),foreign key(tenant_id,document_id) references agro360.fiscal_issuance_documents(tenant_id,id));
create table if not exists agro360.fiscal_issuance_document_events(id uuid primary key default gen_random_uuid(),tenant_id uuid not null,document_id uuid not null,event_type varchar(50) not null,from_status varchar(30),to_status varchar(30),safe_message varchar(1000),provider_protocol varchar(100),occurred_at timestamptz not null default now(),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,foreign key(tenant_id,document_id) references agro360.fiscal_issuance_documents(tenant_id,id));
create table if not exists agro360.fiscal_issuance_document_xml_files(id uuid primary key default gen_random_uuid(),tenant_id uuid not null,document_id uuid not null,storage_reference varchar(500) not null,sha256 char(64) not null,size_bytes bigint not null check(size_bytes>0),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,unique(tenant_id,document_id,sha256),foreign key(tenant_id,document_id) references agro360.fiscal_issuance_documents(tenant_id,id));
...
create index if not exists ix_fiscal_issuance_documents_queue on agro360.fiscal_issuance_documents(tenant_id,status,document_type,created_at desc);
...
insert into agro360.identity_permissions(code,module,description) values ('external-api.manage','API Externa','Gerenciar aplicações, chaves e escopos.'),('fiscal.read','Fiscal','Consultar documentos fiscais e XML autorizado.'),('fiscal.write','Fiscal','Importar documentos e preparar rascunhos fiscais.'),('fiscal.xml.download','Fiscal','Baixar XML fiscal protegido.'),('exports.create','Integrações','Gerar exportações de dados do tenant.') on conflict(code) do update set module=excluded.module,description=excluded.description;
...
create table if not exists agro360.intelligence_operational_recommendations(id uuid primary key default gen_random_uuid(),tenant_id uuid not null references agro360.tenancy_tenants(id),rule_id uuid,module varchar(60) not null,type varchar(40) not null,title varchar(240) not null,reason varchar(1200) not null,suggested_action varchar(1000) not null,severity varchar(12) not null check(severity in('LOW','ATTENTION','HIGH','CRITICAL')),status varchar(16) not null default 'OPEN' check(status in('OPEN','ACCEPTED','REJECTED','IGNORED','ARCHIVED')),entity_type varchar(60) not null,entity_id uuid not null,fingerprint char(64) not null,impact_amount numeric(18,2),due_at timestamptz,assigned_to uuid,decided_at timestamptz,decided_by uuid,decision_reason varchar(1000),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,deleted_at timestamptz,unique(tenant_id,id),foreign key(tenant_id,rule_id) references agro360.intelligence_rules(tenant_id,id));
create unique index if not exists ux_intelligence_operational_recommendation_open on agro360.intelligence_operational_recommendations(tenant_id,fingerprint) where status='OPEN' and deleted_at is null;
create table if not exists agro360.intelligence_operational_recommendation_events(id uuid primary key default gen_random_uuid(),tenant_id uuid not null,recommendation_id uuid not null,event_type varchar(30) not null,reason varchar(1000),task_id uuid,workflow_id uuid,occurred_at timestamptz not null default now(),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,foreign key(tenant_id,recommendation_id) references agro360.intelligence_operational_recommendations(tenant_id,id));
create table if not exists agro360.intelligence_operational_recommendation_sources(id uuid primary key default gen_random_uuid(),tenant_id uuid not null,recommendation_id uuid not null,source_module varchar(60) not null,source_entity_type varchar(60) not null,source_entity_id uuid not null,observed_value varchar(300),reference_value varchar(300),observed_at timestamptz not null,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid not null,updated_by uuid not null,foreign key(tenant_id,recommendation_id) references agro360.intelligence_operational_recommendations(tenant_id,id));
...
create index if not exists ix_intel_operational_recommendations_queue on agro360.intelligence_operational_recommendations(tenant_id,status,severity,module,created_at desc);
create index if not exists ix_intel_operational_recommendations_entity on agro360.intelligence_operational_recommendations(tenant_id,entity_type,entity_id);
...
-- A fundação já cria fleet_assets. Evolua a mesma tabela sem perder os contratos
-- das operações legadas (code/asset_type/identification/current_hour_meter).
alter table agro360.fleet_assets
    ...
    add column if not exists fuel_capacity numeric(14,3),
    add column if not exists main_operator_id uuid,
    add column if not exists acquired_on date,
    add column if not exists acquisition_value numeric(18,2),
    add column if not exists notes text;
alter table agro360.fleet_assets
    drop constraint if exists fk_fleet_assets_asset_type,
    drop constraint if exists fk_fleet_assets_operator,
    drop constraint if exists fk_fleet_assets_cost_center;
alter table agro360.fleet_assets
    add constraint fk_fleet_assets_asset_type foreign key (tenant_id,asset_type_id) references agro360.fleet_asset_types(tenant_id,id),
    add constraint fk_fleet_assets_operator foreign key (tenant_id,main_operator_id) references agro360.fleet_operators(tenant_id,id),
    add constraint fk_fleet_assets_cost_center foreign key (tenant_id,cost_center_id) references agro360.finance_cost_centers(tenant_id,id);
...
alter table agro360.finance_cost_centers
    add column if not exists application varchar(30),
    add column if not exists notes text,
...
commit;
begin;
create table if not exists agro360.export_trading_incoterms(code varchar(3) primary key, name text not null, seller_responsibilities text not null, buyer_responsibilities text not null, active boolean not null default true);
...
create table if not exists agro360.fiscal_purchase_checks(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),purchase_order_id uuid not null,receipt_id uuid,supplier_id uuid not null,fiscal_document_id uuid not null,expected_total numeric(20,2) not null,received_total numeric(20,2) not null,status varchar(20) not null check(status in('PENDING','DIVERGENT','APPROVED','REJECTED')),justification text,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid,unique(tenant_id,id),foreign key(tenant_id,fiscal_document_id) references agro360.fiscal_documents(tenant_id,id),check(expected_total>=0 and received_total>=0));
create table if not exists agro360.fiscal_purchase_check_items(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),purchase_check_id uuid not null,product_id uuid not null,expected_quantity numeric(20,6) not null,received_quantity numeric(20,6) not null,expected_value numeric(20,2) not null,received_value numeric(20,2) not null,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid,foreign key(tenant_id,purchase_check_id) references agro360.fiscal_purchase_checks(tenant_id,id));
create table if not exists agro360.fiscal_divergences(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),purchase_check_id uuid not null,type varchar(30) not null,expected_value numeric(20,6),received_value numeric(20,6),status varchar(20) not null check(status in('OPEN','JUSTIFIED','RESOLVED')),justification text,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid,foreign key(tenant_id,purchase_check_id) references agro360.fiscal_purchase_checks(tenant_id,id));
create table if not exists agro360.fiscal_tax_summaries(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),document_id uuid not null,tax_type varchar(20) not null,tax_base numeric(20,2) not null,tax_rate numeric(7,4),tax_value numeric(20,2) not null,retained boolean not null default false,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid,foreign key(tenant_id,document_id) references agro360.fiscal_documents(tenant_id,id),check(tax_base>=0 and tax_value>=0 and tax_rate between 0 and 100));
create table if not exists agro360.fiscal_stock_movements(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),invoice_id uuid,document_id uuid,product_id uuid not null,lot_id uuid,type varchar(30) not null,quantity numeric(20,6) not null check(quantity>0),status varchar(30) not null,message text,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid);
create table if not exists agro360.fiscal_financial_integrations(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),type varchar(20) not null,reference_id uuid not null,status varchar(30) not null check(status in('PENDING','INTEGRATED','FAILED','NOT_CONFIGURED','CANCELLED')),message text,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),created_by uuid,updated_by uuid);
create table if not exists agro360.fiscal_reports_exports(id uuid primary key,tenant_id uuid not null references agro360.tenancy_tenants(id),report varchar(60) not null,filters jsonb not null default '{}',row_count integer not null default 0,created_at timestamptz not null default now(),created_by uuid);
create index if not exists ix_fiscal_operations_tenant_status on agro360.fiscal_operations(tenant_id,active,name) where deleted_at is null;create index if not exists ix_fiscal_rules_tenant_operation_product on agro360.fiscal_operation_rules(tenant_id,operation_id,product_id,origin_state,destination_state,valid_from) where deleted_at is null;create index if not exists ix_fiscal_invoices_tenant_status_date on agro360.fiscal_invoice_drafts(tenant_id,status,created_at,customer_id,operation_id) where deleted_at is null;create index if not exists ix_fiscal_documents_tenant_status_date on agro360.fiscal_documents(tenant_id,status,issued_at,customer_id,supplier_id) where deleted_at is null;create index if not exists ix_fiscal_checks_tenant_status on agro360.fiscal_purchase_checks(tenant_id,status,created_at,supplier_id);create index if not exists ix_fiscal_divergences_open on agro360.fiscal_divergences(tenant_id,status,created_at);create index if not exists ix_fiscal_stock_status on agro360.fiscal_stock_movements(tenant_id,status,created_at,product_id);create index if not exists ix_fiscal_financial_status on agro360.fiscal_financial_integrations(tenant_id,status,created_at);

insert into agro360.identity_permissions(code,module,description) values('fiscal.read','Fiscal e Faturamento','Consultar dados fiscais.'),('fiscal.write','Fiscal e Faturamento','Cadastrar operações, regras e documentos.'),('fiscal.approve','Fiscal e Faturamento','Confirmar faturamentos e conferências.'),('fiscal.reports','Fiscal e Faturamento','Exportar relatórios fiscais.') on conflict(code) do update set module=excluded.module,description=excluded.description;
...
do $$ begin
 if not exists (select 1 from pg_constraint where conname='ck_identity_users_normalized_document' and conrelid='agro360.identity_users'::regclass) then
  alter table agro360.identity_users add constraint ck_identity_users_normalized_document
    ...
    loop
        ...
        if not exists (
            ...
            where schemaname = 'agro360'
              ...
              -- PostgreSQL limits identifiers to 63 bytes and silently truncates
              -- generated policy names when the table name is long.
              and policyname = left(policy_name, 63))
        then
            execute format(
...
-- Mantém o catálogo do banco em paridade com Agro360.Application.Permissions.
-- Entradas já documentadas preservam módulo e descrição; somente lacunas são
-- completadas com uma descrição canônica neutra.
insert into agro360.identity_permissions(code,module,description)
select code,
       initcap(replace(split_part(code,'.',1),'-',' ')),
       'Permissão canônica usada pelas policies da aplicação.'
from unnest(array[
        'export.read', 'export.write', 'export.approve', 'export.reports', 'fiscal.read',
        'fiscal.write', 'fiscal.approve', 'fiscal.reports', 'properties.read', 'properties.write',
        'agriculture.read', 'agriculture.write', 'inventory.read', 'inventory.move', 'inventory.adjust',
        'livestock.read', 'livestock.write', 'livestock.sell', 'crm.read', 'crm.write',
        'commercial-saas.write', 'customer-success.read', 'commercial.write', 'commercial.read', 'commercial.orders.approve',
        'commercial.customers.override-block', 'commercial.commissions.manage', 'commercial.splits.approve', 'finance.read', 'finance.write',
        'finance.settle', 'finance.reconcile', 'finance.export', 'finance.budget.approve', 'purchasing.read',
        'purchasing.write', 'purchasing.approve', 'purchasing.request', 'purchasing.receive', 'purchasing.homologate',
        'purchasing.export', 'production.read', 'production.write', 'production.release', 'production.operate',
        'production.quality', 'production.cancel', 'production.export', 'fleet.read', 'fleet.write',
        'fleet.meter.override', 'maintenance.read', 'maintenance.write', 'dashboard.read', 'storage.read',
        'storage.write', 'logistics.read', 'logistics.write', 'traceability.read', 'traceability.write',
        'ledger.validate', 'regional-logistics.read', 'regional-logistics.write', 'sales-network.read', 'sales-network.write',
        'sales-network.approve', 'intelligence.read', 'intelligence.write', 'intelligence.strategic', 'intelligence.export',
        'compliance.read', 'compliance.write', 'compliance.approve', 'esg.read', 'esg.write',
        'sustainability.read', 'sustainability.write', 'sustainability.approve', 'sustainability.reports', 'marketplace.read',
        'marketplace.manage', 'partners.manage', 'api-keys.manage', 'integrations.audit', 'integrations.read',
        'integrations.write', 'maps.read', 'maps.write', 'cooperative.read', 'cooperative.write',
        'cooperative.approve', 'rural-hr.read', 'rural-hr.write', 'rural-hr.safety', 'deployment.read',
        'deployment.write', 'documents.read', 'documents.upload', 'documents.download', 'evidences.validate',
        'dossiers.create', 'dossiers.approve', 'certificates.issue', 'certificates.revoke', 'work.read',
        'work.write', 'work.approve', 'mobile.read', 'mobile.write', 'mobile.sync',
        'mobile.conflicts.resolve', 'field-checklists.manage', 'portal.manage', 'portal.access', 'support.read',
        'support.write', 'support.manage', 'sst.read', 'sst.write', 'sst.medical.read',
        'governance.read', 'governance.write', 'governance.export', 'lgpd.manage', 'security.sessions'
    ]::varchar[]) as canonical(code)
on conflict(code) do nothing;

-- Repara códigos publicados por versões intermediárias do instalador. As
-- permissões da aplicação usam o padrão module.action; os vínculos existentes
-- são migrados antes da remoção dos aliases inválidos.
do $$
declare
    repair record;
    legacy_permission_id uuid;
    canonical_permission_id uuid;
begin
    for repair in select * from (values
        ('agro360.agriculture_read','agriculture.read'), ('agro360.agriculture_write','agriculture.write'),
        ('agro360.inventory_read','inventory.read'), ('agro360.inventory_move','inventory.move'), ('agro360.inventory_adjust','inventory.adjust'),
        ('agro360.livestock_read','livestock.read'), ('agro360.livestock_write','livestock.write'), ('agro360.livestock_sell','livestock.sell'),
        ('agro360.commercial_read','commercial.read'), ('agro360.commercial_write','commercial.write'),
        ('agro360.commercial_orders.approve','commercial.orders.approve'),
        ('agro360.commercial_customers.override-block','commercial.customers.override-block'),
        ('agro360.commercial_commissions.manage','commercial.commissions.manage'), ('agro360.commercial_splits.approve','commercial.splits.approve'),
        ('agro360.finance_read','finance.read'),
        ('agro360.intelligence_read','intelligence.read'), ('agro360.intelligence_write','intelligence.write'),
        ('agro360.mobile_read','mobile.read'), ('agro360.mobile_write','mobile.write'), ('agro360.mobile_sync','mobile.sync'),
        ('agro360.mobile_conflicts.resolve','mobile.conflicts.resolve'),
        ('agro360.deployment_read','deployment.read'), ('agro360.deployment_write','deployment.write'),
        ('agro360.documents_read','documents.read'), ('agro360.documents_upload','documents.upload'), ('agro360.documents_download','documents.download'),
        ('agro360.bi_read','bi.read'), ('agro360.bi_export','bi.export'), ('agro360.bi_manage','bi.manage'),
        ('agro360.geo_read','geo.read'), ('agro360.geo_manage','geo.manage'),
        ('field-agro360.mobile_read','field-mobile.read'), ('field-agro360.mobile_write','field-mobile.write'),
        ('field-agro360.mobile_approve','field-mobile.approve'), ('field-agro360.mobile_sync','field-mobile.sync'),
        ('field-agro360.mobile_conflicts.resolve','field-mobile.conflicts.resolve'), ('field-agro360.mobile_reports','field-mobile.reports'),
        ('agro360.storage_read','storage.read'), ('agro360.storage_write','storage.write'),
        ('agro360.logistics_read','logistics.read'), ('agro360.logistics_write','logistics.write'),
        ('agro360.traceability_read','traceability.read'), ('agro360.traceability_write','traceability.write'),
        ('agro360.ledger_validate','ledger.validate'),
        ('regional-agro360.logistics_read','regional-logistics.read'), ('regional-agro360.logistics_write','regional-logistics.write'),
        ('agro360.compliance_read','compliance.read'), ('agro360.compliance_write','compliance.write'),
        ('agro360.compliance_approve','compliance.approve'), ('agro360.esg_read','esg.read'), ('agro360.esg_write','esg.write'),
        ('agro360.integrations_read','integrations.read'), ('agro360.integrations_write','integrations.write'),
        ('agro360.cooperative_read','cooperative.read'), ('agro360.cooperative_write','cooperative.write'),
        ('agro360.cooperative_approve','cooperative.approve'),
        ('rural-agro360.hr_read','rural-hr.read'), ('rural-agro360.hr_write','rural-hr.write'), ('rural-agro360.hr_safety','rural-hr.safety'),
        ('agro360.fiscal_read','fiscal.read'), ('agro360.fiscal_write','fiscal.write'),
        ('agro360.fiscal_xml.download','fiscal.xml.download'), ('agro360.fiscal_approve','fiscal.approve'), ('agro360.fiscal_reports','fiscal.reports'),
        ('agro360.intelligence_rules.manage','intelligence.rules.manage'),
        ('agro360.intelligence_recommendations.decide','intelligence.recommendations.decide'),
        ('agro360.intelligence_assistant.use','intelligence.assistant.use'),
        ('agro360.intelligence_strategic','intelligence.strategic'), ('agro360.intelligence_export','intelligence.export'),
        ('agro360.support_read','support.read'), ('agro360.support_write','support.write'), ('agro360.support_manage','support.manage'),
        ('agro360.sst_read','sst.read'), ('agro360.sst_write','sst.write'), ('agro360.sst_medical.read','sst.medical.read'),
        ('agro360.fleet_read','fleet.read'), ('agro360.fleet_write','fleet.write'), ('agro360.fleet_meter.override','fleet.meter.override'),
        ('agro360.finance_settle','finance.settle'), ('agro360.finance_reconcile','finance.reconcile'),
        ('agro360.finance_export','finance.export'), ('agro360.finance_budget.approve','finance.budget.approve'),
        ('agro360.purchasing_request','purchasing.request'), ('agro360.purchasing_receive','purchasing.receive'),
        ('agro360.purchasing_homologate','purchasing.homologate'), ('agro360.purchasing_export','purchasing.export'),
        ('agro360.production_read','production.read'), ('agro360.production_write','production.write'),
        ('agro360.production_release','production.release'), ('agro360.production_operate','production.operate'),
        ('agro360.production_quality','production.quality'), ('agro360.production_cancel','production.cancel'),
        ('agro360.production_export','production.export')
    ) as aliases(legacy_code, canonical_code)
    loop
        select id into legacy_permission_id from agro360.identity_permissions where code=repair.legacy_code;
        select id into canonical_permission_id from agro360.identity_permissions where code=repair.canonical_code;
        if legacy_permission_id is not null and canonical_permission_id is not null then
            insert into agro360.identity_role_permissions(tenant_id,role_id,permission_id)
            select tenant_id,role_id,canonical_permission_id
            from agro360.identity_role_permissions
            where permission_id=legacy_permission_id
            on conflict do nothing;
            update agro360.platform_menu_items
            set permission_code=repair.canonical_code
            where permission_code=repair.legacy_code;
            delete from agro360.identity_role_permissions where permission_id=legacy_permission_id;
            delete from agro360.identity_permissions where id=legacy_permission_id;
        end if;
        legacy_permission_id := null;
        canonical_permission_id := null;
    end loop;
end $$;
...
-- Sprint Comercial Agro 360: entregas e previsões são registros operacionais; não representam baixa financeira.
begin;
create table if not exists agro360.commercial_deliveries(
 id uuid primary key, tenant_id uuid not null references agro360.tenancy_tenants(id), order_id uuid not null,
 name varchar(80) not null, carrier varchar(180), own_fleet boolean not null default false, delivery_address text not null,
 expected_at timestamptz not null, status varchar(24) not null default 'SCHEDULED', evidence_url varchar(1000), occurrence_description text,
 cancellation_reason text, completed_at timestamptz, completed_by uuid, created_at timestamptz not null default now(),
 updated_at timestamptz not null default now(), created_by uuid not null, updated_by uuid, deleted_at timestamptz,
 unique(tenant_id,id), unique(tenant_id,name), foreign key(tenant_id,order_id) references agro360.sales_orders(tenant_id,id),
 check(status in('SCHEDULED','PICKING','IN_TRANSIT','DELIVERED','INCIDENT','CANCELLED')),
 check(status<>'INCIDENT' or nullif(trim(occurrence_description),'') is not null),
 check(status<>'CANCELLED' or nullif(trim(cancellation_reason),'') is not null),
 check(status<>'DELIVERED' or (completed_at is not null and completed_by is not null)));
create table if not exists agro360.commercial_billing_forecasts(
 id uuid primary key, tenant_id uuid not null references agro360.tenancy_tenants(id), order_id uuid not null, customer_id uuid not null,
 name varchar(80) not null, installment_number int not null, installment_count int not null, amount numeric(18,2) not null,
 due_date date not null, cost_center_id uuid, category_id uuid, status varchar(20) not null default 'EXPECTED',
 created_at timestamptz not null default now(), updated_at timestamptz not null default now(), created_by uuid not null, updated_by uuid, deleted_at timestamptz,
 unique(tenant_id,id), unique(tenant_id,order_id,installment_number), foreign key(tenant_id,order_id) references agro360.sales_orders(tenant_id,id),
 foreign key(tenant_id,customer_id) references agro360.crm_customers(tenant_id,id), check(amount>0),
 check(installment_number>0 and installment_count>0 and installment_number<=installment_count), check(status in('EXPECTED','OVERDUE','CANCELLED','SETTLED')));
create table if not exists agro360.commercial_events(
 id uuid primary key, tenant_id uuid not null references agro360.tenancy_tenants(id), aggregate_type varchar(40) not null,
 aggregate_id uuid not null, event_type varchar(60) not null, payload jsonb not null default '{}', created_at timestamptz not null default now(), created_by uuid not null);
create index if not exists ix_commercial_deliveries_tenant_status_date on agro360.commercial_deliveries(tenant_id,status,expected_at) where deleted_at is null;
create index if not exists ix_commercial_deliveries_order on agro360.commercial_deliveries(tenant_id,order_id);
create index if not exists ix_commercial_forecasts_tenant_status_date on agro360.commercial_billing_forecasts(tenant_id,status,due_date) where deleted_at is null;
...
-- Recebimento de compras: idempotencia e vinculos transacionais com estoque/financeiro.
begin;
...
insert into agro360.platform_schema_versions(version,description,installed_at)
values('6.4.1','Recebimento de compras integrado a estoque e previsao financeira idempotente',now())
on conflict(version) do update set description=excluded.description;
...
begin;
alter table agro360.procurement_receipts add column if not exists request_fingerprint varchar(64);
do $$ begin alter table agro360.procurement_receipts add constraint ck_procurement_receipt_fingerprint check (request_fingerprint is null or request_fingerprint ~ '^[0-9a-f]{64}$'); exception when duplicate_object then null; end $$;
insert into agro360.identity_permissions(code,module,description) values ('purchasing.receipts.override-excess','Compras e Suprimentos','Autorizar recebimento acima da quantidade do pedido.') on conflict(code) do update set module=excluded.module,description=excluded.description;
...
-- === migration 071_fulfillment_delivery_journey ===
begin;
...
-- === migration 088_fulfillment_reservation_integrity ===
...
update agro360.fulfillment_reservations set consumed_quantity=quantity
where status='CONSUMED' and consumed_quantity=0 and released_quantity=0;
alter table agro360.fulfillment_reservations drop constraint if exists ck_fulfillment_reservation_quantities;
alter table agro360.fulfillment_reservations add constraint ck_fulfillment_reservation_quantities check(consumed_quantity>=0 and released_quantity>=0 and consumed_quantity+released_quantity<=quantity);
create index if not exists ix_fulfillment_reservation_order_item on agro360.fulfillment_reservations(tenant_id,order_item_id,status);
create index if not exists ix_fulfillment_reservation_lot on agro360.fulfillment_reservations(tenant_id,stock_lot_id,status);
insert into agro360.platform_schema_versions(version,description,installed_at) values('8.8.0','Integridade de reservas e expedições parciais',now()) on conflict(version) do nothing;

-- === migration 089_after_sales_returns ===
...
do $$
begin
  perform agro360.platform_enable_tenant_rls('agro360.quality_inspection_event_intents');
end $$;
...
-- 9.5.0: Genealogia operacional da safra até produção e expedição (AG-E6-GEN-001).
...
create table if not exists agro360.public_trace_publications (
    ...
    created_at timestamptz not null default now(), created_by uuid not null,
    updated_at timestamptz not null default now(), updated_by uuid not null,
    unique(tenant_id,id), unique(public_code), unique(tenant_id,idempotency_key),
...
create index if not exists ix_public_trace_entity on agro360.public_trace_publications(tenant_id,entity_type,entity_id);
create index if not exists ix_public_trace_active_code on agro360.public_trace_publications(public_code) where status='PUBLISHED';
...
declare
 ...
 user_id uuid; role_id uuid; profile_id uuid;
...
begin
 ...
 else update agro360.tenancy_tenants set name='Agro360 Plataforma',slug='agro360-platform',timezone_id='America/Sao_Paulo',status=1,plan_code='ENTERPRISE',deleted_at=null,updated_at=now() where id=platform_id; end if;
 ...
 else update agro360.tenancy_tenants set name='Fazenda Santa Clara',slug='fazenda-santa-clara',timezone_id='America/Sao_Paulo',status=1,plan_code='ENTERPRISE',deleted_at=null,updated_at=now() where id=santa_id; end if;
 ...
 else update agro360.tenancy_tenants set name='Cooperativa Vale Verde',slug='cooperativa-vale-verde',timezone_id='America/Sao_Paulo',status=1,plan_code='GROWTH',deleted_at=null,updated_at=now() where id=vale_id; end if;
 ...
 else update agro360.tenancy_tenants set name='Fazenda Bloqueada Teste',slug='fazenda-bloqueada-teste',timezone_id='America/Sao_Paulo',status=3,plan_code='ENTERPRISE',deleted_at=null,updated_at=now() where id=blocked_id; end if;
 ...
 on conflict(id) do update set legal_name=excluded.legal_name,trade_name=excluded.trade_name,
  ...
  legal_contact=excluded.legal_contact,plan_id=excluded.plan_id,status=excluded.status,
  block_reason=excluded.block_reason,deleted_at=null,updated_at=now();

 -- Reaplicacao tambem reconcilia o conjunto contratado (remove direitos que
 -- nao pertencem mais a fixture e reativa exatamente os listados abaixo).
 delete from agro360.platform_tenant_module_entitlements
 where tenant_id in (santa_id,vale_id,blocked_id);
 ...
 select id into user_id from agro360.identity_users where tenant_id=platform_id and lower(email)='superadmin@agro360.local' and deleted_at is null;
 if user_id is null then user_id:=gen_random_uuid(); end if;
 ...
 values(user_id,platform_id,'Super Admin Agro360','superadmin@agro360.local',admin_hash,'ACTIVE','00000000000','CPF',false,false)
 ...
 insert into agro360.identity_user_roles values(platform_id,user_id,role_id) on conflict do nothing;
 ...
 insert into agro360.platform_super_admins(id,user_id,active) values(gen_random_uuid(),user_id,true)
 ...
 select id into user_id from agro360.identity_users where tenant_id=santa_id and lower(email) in ('admin.santaclara@agro360.local','admin.cliente@agro360.local') and deleted_at is null order by (lower(email)='admin.santaclara@agro360.local') desc limit 1;
 if user_id is null then user_id:=gen_random_uuid(); end if;
 update agro360.identity_users set name='Admin Fazenda Santa Clara',email='admin.santaclara@agro360.local',password_hash=client_hash,status='ACTIVE',normalized_document='11222333000181',document_type='CNPJ',must_change_password=false,mfa_enabled=false,deleted_at=null,updated_at=now() where id=user_id;
 if not found then insert into agro360.identity_users(id,tenant_id,name,email,password_hash,status,normalized_document,document_type,must_change_password,mfa_enabled) values(user_id,santa_id,'Admin Fazenda Santa Clara','admin.santaclara@agro360.local',client_hash,'ACTIVE','11222333000181','CNPJ',false,false); end if;
 ...
 select id into user_id from agro360.identity_users where tenant_id=santa_id and lower(email)='operador.santaclara@agro360.local' and deleted_at is null;
 if user_id is null then user_id:=gen_random_uuid(); end if;
 ...
 foreach vale_id in array array[vale_id,blocked_id] loop
  ...
  select id into user_id from agro360.identity_users where tenant_id=vale_id and lower(email)=case when vale_id=blocked_id then 'admin.bloqueado@agro360.local' else 'admin.valeverde@agro360.local' end and deleted_at is null;
  if user_id is null then user_id:=gen_random_uuid(); end if;
  insert into agro360.identity_users(id,tenant_id,name,email,password_hash,status,normalized_document,document_type,must_change_password,mfa_enabled)
