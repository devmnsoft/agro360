...
-- Reconciliar uma role preexistente tambem e necessario: IF NOT EXISTS, sozinho,
-- nao comprova que ela foi criada com o perfil seguro esperado pela aplicacao.
ALTER ROLE agro360_app NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
...
-- Incremental source: 104_saas_administration_lifecycle.sql
...
-- Origem canônica: database/migrations/108_proposal_conversion_currency.sql
begin;
...
alter table agro360.sales_orders add constraint ck_sales_orders_currency_format check(currency is null or currency ~ '^[A-Z]{3}$') not valid;
alter table agro360.sales_orders validate constraint ck_sales_orders_currency_format;
insert into agro360.platform_schema_versions(version,description,installed_at) values('10.8.0','Moeda canônica e reconciliação na conversão de propostas',now()) on conflict(version) do nothing;
commit;
begin;
...
-- Origem canônica: database/migrations/110_fulfillment_operational_closure.sql
begin;
...
-- Origem canônica: database/migrations/112_fulfillment_reopen_after_snapshot.sql
begin;
...
alter table agro360.fulfillment_preparation_reopens drop constraint if exists ck_fulfillment_reopen_after_quantities;
alter table agro360.fulfillment_preparation_reopens add constraint ck_fulfillment_reopen_after_quantities
