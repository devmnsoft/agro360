...
create table if not exists platform_tenant_module_entitlements (tenant_id uuid not null references platform_tenants(id), module_id uuid not null references platform_module_catalog(id), status varchar(20) not null check(status in ('CONTRACTED','ACTIVE','BLOCKED','TRIAL','DELINQUENT','SUSPENDED')), reason text, activated_at timestamptz, created_at timestamptz not null default now(), updated_at timestamptz, created_by uuid, updated_by uuid, primary key(tenant_id,module_id));
create index if not exists ix_platform_tenant_module_entitlements_status on platform_tenant_module_entitlements(tenant_id,status);
...
do $$ declare t text; begin foreach t in array array['platform_tenant_settings','platform_tenant_status_events','platform_profiles','platform_profile_permissions','platform_user_profiles','platform_tenant_module_entitlements','platform_billing_charges','platform_billing_events'] loop execute format('alter table %I enable row level security',t); execute format('drop policy if exists tenant_isolation on %I',t); execute format('create policy tenant_isolation on %I using (tenant_id = nullif(current_setting(''app.tenant_id'',true),'''')::uuid) with check (tenant_id = nullif(current_setting(''app.tenant_id'',true),'''')::uuid)',t); end loop; end $$;
