-- Agro 360 v0.2.0 - instalador autônomo para PostgreSQL 14+
BEGIN;
DO $$ BEGIN IF current_setting('server_version_num')::integer < 140000 THEN RAISE EXCEPTION 'Agro 360 requer PostgreSQL 14 ou superior'; END IF; END $$;
...
-- migration: 002_operational_views.sql
...
-- migration: 003_application_role.sql
do $$
declare
    schema_name text;
...
-- migration: 004_outbox_operational_hardening.sql
...
create table if not exists platform.installed_releases (version varchar(40) primary key, installed_at timestamptz not null default now());
insert into platform.installed_releases(version) values ('0.2.0') on conflict (version) do nothing;
COMMIT;
