...
Atualizado em 2026-09-09. Este documento registra somente evidências reproduzíveis; presença de arquivo, rota ou tela não equivale a fluxo homologado.

## Consolidação 2026-09-09 — banco, acesso e recebimento

- Referência integrada: `origin/main` em `2b83165497c9e65e5c4e651a1ba7cc4c5b8ec1d3`, sobre o trabalho local `74a2d5b0d016606e9ee957081b0fa91bb21fa110`, no ramo `codex/consolidar-avanco-compras`. `main` não foi reescrita e nenhum push/deploy foi feito.
- **AG-E0-003 parcial:** a cadeia limpa agora atravessa `006a` e as bridges versionadas 029/036a/040a/042a/046a/048a, materializa o baseline `agro360` com checksum e chega à versão atual. Reexecução do migrator foi acrescentada ao gate. Uma base histórica com `tenancy.tenants` populada é recusada antes de criar tabelas canônicas: a conversão de dados ainda precisa ser implementada/homologada e não é declarada concluída.
- **AG-E1-003 implementada sem homologação concorrente:** criação, aceite de convite e reativação usam a mesma regra de limite sob lock consultivo por tenant; perfis e autoridade delegável do emissor são revalidados no aceite; último administrador e sessões continuam protegidos.
- **AG-E2-001 parcial:** `AccessDecision` é compartilhada por `PermissionAuthorization` e `IdentityService`, com vigência, bloqueios, overrides e dependências. Menu deriva das permissões emitidas; rotas/exports passam pelas policies. Jobs e todo o ciclo comercial ainda não foram homologados.
- **AG-E4-002/AG-E5-001 implementadas sem homologação específica:** recebimento grava estoque/previsão na mesma transação, rejeita excesso acumulado inclusive em linhas repetidas, exige permissão específica no servidor, distingue serviço, quarentena e liberação, valida unidade/lote/validade, compara payload no retry e cancela por compensação auditada. A página de Compras expõe liberar/reprovar/cancelar conforme permissão.
- Controles concorrentes removidos: `docs/CHECKPOINT.md`, `docs/EXECUTION-PLAN.md` e `docs/FEATURE-MATRIX.md`. Permanecem somente este checkpoint, `docs/execucao/EXECUTION-PLAN.md` e a matriz v0.2.0.

### Evidência desta consolidação

- `dotnet restore MNSOFT.Agro360.sln`: aprovado.
- `dotnet build MNSOFT.Agro360.sln --no-restore`: aprovado, zero avisos e zero erros.
- `pwsh -File scripts/verify-e0.ps1 -CheckMigrations ...`: aprovado em `artifacts/e0-a872c8e263f6446a8d83da3ecb52fb9b/`. Cobriu instalação/reinstalação SQL, health, OpenAPI/Swagger, login/dashboard/refresh/replay/logout, Web HTTP, 118 testes com PostgreSQL, cadeia incremental limpa, replay idempotente e bloqueio seguro da base histórica populada.
- Verificações finais: build Release com 0 erros/avisos, `dotnet format --verify-no-changes`, validador do SQL consolidado, 648 operações HTTP únicas, shell offline, sintaxe JavaScript e varredura de credenciais fixas aprovados.
- Navegador real em `http://127.0.0.1:52345/Procurement`: página e modal de login renderizaram em viewport desktop e 390×844; labels/ajudas eram acessíveis. Como a API descartável do gate já estava encerrada, as listas mostraram falha de rede e a jornada autenticada não foi homologada visualmente.
- Não foram criadas classes de teste. Uma asserção da classe existente `LoginExperienceTests` foi ajustada para a decisão central.
- Não homologado nesta rodada: navegador autenticado e responsivo; dois tenants/usuário restrito; módulo expirado/bloqueado; concorrência real de convites/admin; cenários E2E específicos do recebimento; upgrade de base legada populada; Data Protection persistida e acesso assistido.
...
> Continue em `C:\MNSOFT\agro360`. Leia os controles canônicos. Revalide o gate final da consolidação e conclua AG-E0-003 em uma cópia populada da base histórica: implemente mapeamento de dados para `agro360`, compare contagens/vínculos/queries e só então remova o bloqueio preventivo do Migrator. Depois homologue concorrência de AG-E1-003 e os cenários E2E de AG-E5-001. Não faça push.
