...
| AG-E5-001 / Compras | `Procurement`, compras → `ProcurementController` e `OperationsController` → `ProcurementContracts`/`ProcurementService`, `OperationsService` → `procurement_*`, `purchasing_*` | purchasing.* / purchasing | **parcial**; dois cadastros de fornecedor; provar recebimento → estoque/título atômico |
