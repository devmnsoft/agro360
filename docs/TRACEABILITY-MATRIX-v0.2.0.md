...
| AG-E0-003 | Operação: atualização de versão com dados | `Agro360.Migrator/Program.cs` → baseline canônico registrado; `006a` e bridges 029/036a/040a/042a/046a/048a preservam migrations publicadas | **parcial**: cadeia limpa e replay chegam à versão atual em PostgreSQL real. Base legada populada é bloqueada antes do baseline; conversão e comparação dos dados ainda não foram homologadas |
| AG-E0-004 | Integração do worktree | merge de `origin/main` `2b831654` com o commit local `74a2d5b` no ramo `codex/consolidar-avanco-compras` | **implementado não validado até o estado final**: conflitos foram resolvidos preservando SaaS/propriedades e compras; controles documentais duplicados foram removidos; gate final deve suceder o último diff |
...
| AG-E1-002 | SuperAdmin → MFA → suporte assistido | `SuperAdminProvisioner` e `IdentityService` validam segredo/TOTP; administração global usa role e registro ativo | **parcial**: TOTP existe; troca inicial, Data Protection persistida e sessão assistida completa ainda não foram percorridas nesta consolidação |
| AG-E1-003 | Admin cliente → usuários/perfis/convites | `/Saas`, `saas.js` → `SaasControllers`/`SaasService`; lock por tenant, limite central, delegação por permissões e revogação | **implementado não validado**: código cobre criação/aceite/reativação, emissor alterado e último admin; falta teste concorrente/autenticado multi-tenant |
...
| AG-E2-001 | Cliente/MNSOFT → catálogo/contrato | `AccessDecision` resolve assinatura vigente, plano legado, entitlement, marketplace, override, negações e dependências; `IdentityService` e `PermissionAuthorization` reutilizam a decisão | **parcial**: backend/token centralizados e revogação consultada no banco; menus refletem token filtrado. Jobs e fluxo completo de preço/aceite/snapshot ainda não usam/provam o contrato único |
...
| AG-E5-001 / Compras | `Procurement` → `ProcurementController` → `ProcurementContracts`/`ProcurementService` → `procurement_*`, `inventory_*`, `finance_*`; `purchasing_*` fica legado sem novas escritas | purchasing.* + permissões específicas de excesso/inspeção/cancelamento / purchasing | **implementado não validado**; transação, locks, idempotência por payload, quarentena/liberação, movimento, parcelas e cancelamento compensatório existem. Falta executar a matriz de concorrência/rollback/duplicidade e definir migração/leitura do legado |
| AG-E5-002 / Comercial | `Commercial` → `CommercialController`, `Commercial360Controller`, `AgroSalesController` → contratos/`CommercialService`, `Commercial360Service`, `SalesService` → `commercial_*`, `sales_*`, recebíveis | commercial.* / commercial | **implementado não validado**; venda/reserva/entrega/financeiro sem duplicação |
...
| AG-E7-001 / Agroindústria | `Production` → `IndustrialProductionController` → contratos/`IndustrialProductionService` → `production_*` | production.* / agroindustry | **implementado não validado**; receita/ordem/consumo/perda/lote e bloqueio qualidade |
...
| AG-E9-006 / Idiomas/UX | layout/scripts/CSS, `docs/MULTILANGUAGE.md` | visualização não concede permissão | **parcial**; navegador verificou login/ajuda/erro, não responsividade completa. Layout contém clima 29° e rótulo online fixos; não são medição real |
