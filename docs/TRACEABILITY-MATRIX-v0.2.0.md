...
| AG-E5-002 / Comercial | `Commercial` → `CommercialController`, `Commercial360Controller`, `AgroSalesController` → contratos/`CommercialService`, `Commercial360Service`, `SalesService` → `commercial_*`, `sales_*`, recebíveis | commercial.* / commercial | **parcial**; transição do pedido normalizada, fechada e bloqueada em transação; alçada de desconto vem da tabela vigente do segmento. Pendente homologar banco, override, reserva/entrega/financeiro |
