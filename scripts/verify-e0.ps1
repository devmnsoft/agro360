...
try {
    ...
    if ($CheckMigrations) {
        ...
        Set-TaskEnvironment PGDATABASE 'agro360_incremental_test'
        ...
        dotnet src/Hosts/Agro360.Migrator/bin/Release/net10.0/Agro360.Migrator.dll migrate --migrations "$root\database\migrations" > "$evidence\migrator-replay.log" 2>&1
        Assert-Exit 'reexecução idempotente do migrator'
        Invoke-Sql "do `$`$ begin if to_regclass('agro360.tenancy_tenants') is null or not exists(select 1 from agro360.platform_schema_migrations where version='__canonical_baseline_v6_5_1') then raise exception 'baseline canônico ou histórico ausente'; end if; end `$`$;"

        Invoke-Sql 'create database agro360_populated_legacy_test;'
        Set-TaskEnvironment PGDATABASE 'agro360_populated_legacy_test'
        Set-TaskEnvironment ConnectionStrings__Agro360 ($connection.Replace('Database=agro360_e0_test;', 'Database=agro360_populated_legacy_test;'))
        & "$PostgresBin\psql.exe" -X -q -v ON_ERROR_STOP=1 -f database/migrations/001_foundation.sql > "$evidence\legacy-foundation.log" 2>&1
        Assert-Exit 'fundação histórica para gate de dados'
        Invoke-Sql "insert into tenancy.tenants(id,name,slug) values('90000000-0000-0000-0000-000000000001','Tenant legado preservado','tenant-legado-preservado');"
        dotnet src/Hosts/Agro360.Migrator/bin/Release/net10.0/Agro360.Migrator.dll migrate --migrations "$root\database\migrations" > "$evidence\migrator-populated-legacy.log" 2>&1
        if ($LASTEXITCODE -ne 1) { throw "Base legada populada não foi bloqueada antes da conversão homologada. Evidência: $evidence" }
        Invoke-Sql "do `$`$ begin if to_regclass('agro360.tenancy_tenants') is not null then raise exception 'schema canônico materializado sobre base legada populada'; end if; end `$`$;"
        Write-Output 'PASS base legada populada bloqueada sem materialização canônica parcial'
    }
