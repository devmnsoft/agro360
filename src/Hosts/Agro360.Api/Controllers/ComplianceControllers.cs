...
public sealed class ComplianceController(IComplianceService service, ILogger<ComplianceController> logger) : ControllerBase
{
    ...
    [HttpGet("non-conformities/{id:guid}")] public Task<NonConformityDetail> NonConformity(Guid id, CancellationToken ct) => service.NonConformityAsync(id, ct);
    [HttpGet("non-conformities/{id:guid}/verifications")] public Task<IReadOnlyList<EffectivenessVerification>> Verifications(Guid id, CancellationToken ct) => service.VerificationsAsync(id, ct);
    [HttpPost("non-conformities/{id:guid}/verifications"), Authorize(Policy = Permissions.ComplianceApprove)] public Task<IActionResult> VerifyEffectiveness(Guid id, EffectivenessVerificationCommand x, CancellationToken ct) => Create(() => service.AddVerificationAsync(id, x, ct), $"non-conformities/{id}/verifications");
    [HttpGet("non-conformities/{id:guid}/related-cases")] public Task<IReadOnlyList<RelatedCase>> RelatedCases(Guid id, [FromQuery] RelatedCaseQuery query, CancellationToken ct) => service.RelatedCasesAsync(id, query, ct);
    [HttpPost("non-conformities/{id:guid}/related-cases"), Authorize(Policy = Permissions.ComplianceWrite)] public async Task<IActionResult> LinkRelatedCase(Guid id, RelatedCaseLinkCommand x, CancellationToken ct) { await Boundary("link-related-case", () => service.LinkRelatedCaseAsync(id, x, ct)); return NoContent(); }
    ...
    [HttpPost("non-conformities/{id:guid}/close"), Authorize(Policy = Permissions.ComplianceApprove)] public async Task<IActionResult> CloseNc(Guid id, NonConformityTransitionCommand x, CancellationToken ct) { await Boundary("close-nc", () => service.CloseNonConformityAsync(id, x.Reason, x.ExpectedVersion, ct)); return NoContent(); }
