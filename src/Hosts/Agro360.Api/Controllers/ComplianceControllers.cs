...
public sealed class ComplianceController(IComplianceService service,ILogger<ComplianceController> logger):ControllerBase
{
 ...
 [HttpPost("lots/{id:guid}/{action:regex(^block|unblock$)}"),Authorize(Policy=Permissions.ComplianceApprove)] public async Task<IActionResult> Lot(Guid id,string action,ActionCommand x,CancellationToken ct){await Boundary("lot-decision",()=>service.SetLotBlockAsync(id,action=="block",x.Reason,ct));return NoContent();}
 ...
 [HttpPost("certifications/{id:guid}/{action:regex(^approve|reject$)}"),Authorize(Policy=Permissions.ComplianceApprove)] public async Task<IActionResult> CertificationDecision(Guid id,string action,ActionCommand x,CancellationToken ct){await Boundary("certification-decision",()=>service.DecideCertificationAsync(id,action=="approve",x.Reason,ct));return NoContent();}
