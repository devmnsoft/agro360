...
public sealed class CooperativeController(ICooperativeService service,ILogger<CooperativeController> logger):CooperativeControllerBase(service)
{
 ...
 private async Task<T> Boundary<T>(Func<Task<T>> action){try{return await action();}catch(Exception ex){logger.LogError(ex,"Cooperative API boundary failed");throw;}}
}
