...
public sealed class CrmSaasController(ICrmSaasService service):ControllerBase
{
 ...
 [HttpPost("opportunities"),Authorize(Policy=Permissions.CrmWrite)] public async Task<IActionResult> Opportunity(CrmOpportunityCommand x,CancellationToken ct){var id=await service.CreateOpportunityAsync(x,ct);return Created($"/api/crm/opportunities/{id}",new{id});}
