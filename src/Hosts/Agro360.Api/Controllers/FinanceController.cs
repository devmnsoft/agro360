...
public sealed class FinanceController(IFinanceService service, ILogger<FinanceController> logger) : ControllerBase
{
    ...
    [HttpPost("payables/{id:guid}/pay"), Authorize(Policy=Permissions.FinanceSettle)] public async Task<IActionResult> Pay(Guid id,SettlementCommand x,CancellationToken ct){await service.SettleAsync(true,id,x,ct);return NoContent();}
    ...
    [HttpPost("receivables/{id:guid}/receive"), Authorize(Policy=Permissions.FinanceSettle)] public async Task<IActionResult> Receive(Guid id,SettlementCommand x,CancellationToken ct){await service.SettleAsync(false,id,x,ct);return NoContent();}
    ...
    [HttpGet("dashboard"), Authorize(Policy=Permissions.FinanceRead)] public async Task<IActionResult> Dashboard(CancellationToken ct){ApiLogMessages.FinanceDashboardRequested(logger);return Ok(await service.DashboardAsync(ct));}
    [HttpGet("budgets"), Authorize(Policy=Permissions.FinanceRead)] public Task<IReadOnlyList<dynamic>> Budgets(CancellationToken ct)=>service.ListBudgetsAsync(ct);
    [HttpPost("budgets"), Authorize(Policy=Permissions.FinanceWrite)] public async Task<IActionResult> AddBudget(BudgetCommand x,CancellationToken ct)=>Created("api/finance/budgets",new{id=await service.CreateBudgetAsync(x,ct)});
    [HttpPost("budgets/{id:guid}/approve"), Authorize(Policy=Permissions.FinanceBudgetApprove)] public async Task<IActionResult> ApproveBudget(Guid id,CancellationToken ct){await service.ApproveBudgetAsync(id,ct);return NoContent();}
    [HttpPost("reconciliations"), Authorize(Policy=Permissions.FinanceReconcile)] public async Task<IActionResult> Reconcile(ReconciliationCommand x,CancellationToken ct)=>Created("api/finance/reconciliations",new{id=await service.ReconcileAsync(x,ct)});
    [HttpPost("reconciliations/{id:guid}/reverse"), Authorize(Policy=Permissions.FinanceReconcile)] public async Task<IActionResult> ReverseReconciliation(Guid id,CancelFinanceCommand x,CancellationToken ct){await service.ReverseReconciliationAsync(id,x.Reason,ct);return NoContent();}
}
