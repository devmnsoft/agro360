...
public sealed class FleetController(IFleetService service,ILogger<FleetController> logger):ControllerBase
{
 ...
 private async Task<IActionResult> Created(string route,Func<Task<Guid>> action){try{var id=await action();return Created($"/api/fleet/{route}/{id}",new{id});}catch(Exception ex){logger.LogError(ex,"Fleet operation failed: {Operation}",route);throw;}}
