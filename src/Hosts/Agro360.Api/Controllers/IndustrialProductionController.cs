...
public sealed class IndustrialProductionController(IIndustrialProductionService service) : ControllerBase
{
    ...
    [HttpPost("consumptions"), Authorize(Policy = Permissions.ProductionOperate)] public async Task<IActionResult> Consume(ProductionMaterialConsumptionCommand x, CancellationToken ct) => Created("api/production/consumptions", new { id = await service.ConsumeAsync(x, ct) });
