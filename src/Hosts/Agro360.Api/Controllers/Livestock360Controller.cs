...
public sealed class Livestock360Controller(ILivestock360Service service, ILivestockService animals) : ControllerBase
{
 ...
 [HttpPost("api/livestock/animals/{id:guid}/{action:regex(^(death|sale)$)}"),Authorize(Policy=Permissions.LivestockWrite)] public async Task<IActionResult> Status(Guid id,string action,AnimalStatusCommand x,CancellationToken ct){await service.ChangeAnimalStatusAsync(id,action=="death"?"DEAD":"SOLD",x,ct);return NoContent();}
 ...
 [HttpPost("api/pastures/paddocks/{id:guid}/{action:regex(^(occupy|release)$)}"),Authorize(Policy=Permissions.LivestockWrite)] public async Task<IActionResult> PaddockMove(Guid id,string action,PaddockMovementCommand x,CancellationToken ct){await service.MovePaddockAsync(id,action=="occupy",x,ct);return NoContent();}
