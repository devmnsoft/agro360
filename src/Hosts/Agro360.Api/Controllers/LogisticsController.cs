...
public sealed class LogisticsController(ILogisticsService service) : ControllerBase
{
    ...
    [HttpPost("fulfillment/returns/{id:guid}/receipts"), Authorize(Policy = Permissions.LogisticsWrite)] public async Task<IActionResult> ReceiveReturn(Guid id, ReceiveReturnCommand command, CancellationToken ct) { var receiptId = await service.ReceiveReturnAsync(id, command, ct); return Created($"api/logistics/trips/fulfillment/returns/{id}/receipts/{receiptId}", new { id = receiptId }); }
    [HttpPost("fulfillment/returns/{id:guid}/decisions"), Authorize(Policy = Permissions.LogisticsWrite)] public async Task<IActionResult> DecideReturn(Guid id, DecideReturnCommand command, CancellationToken ct) { var decisionId = await service.DecideReturnAsync(id, command, ct); return Created($"api/logistics/trips/fulfillment/returns/{id}/decisions/{decisionId}", new { id = decisionId }); }
}
