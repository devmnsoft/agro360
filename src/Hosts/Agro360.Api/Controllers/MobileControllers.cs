...
[ApiController, Route("api/mobile"), Authorize]
public sealed class MobileController(IMobileService service) : ControllerBase
{
    ...
    [HttpPost("sync")] public Task<dynamic> Sync(SyncCommand command,CancellationToken ct)=>service.SyncAsync(command,ct);
...
public sealed class EvidencesController(IMobileService service):ControllerBase
{
    ...
    [HttpPost, RequestSizeLimit(10_500_000)] public async Task<IActionResult> Post(EvidenceCommand command,CancellationToken ct)=>Created("api/evidences",new{id=await service.AddEvidenceAsync(command,ct)});
}

[ApiController, Route("api/geolocation/events"), Authorize]
public sealed class GeolocationController(IMobileService service):ControllerBase
{
    ...
    [HttpPost] public async Task<IActionResult> Post(GeolocationCommand command,CancellationToken ct)=>Created("api/geolocation/events",new{id=await service.AddLocationAsync(command,ct)});
}

[ApiController, Route("api/qrcode")]
public sealed class QrCodeController(IMobileService service):ControllerBase
{
    [HttpPost("generate"),Authorize] public Task<dynamic> Generate(QrGenerateCommand command,CancellationToken ct)=>service.GenerateQrAsync(command,ct);
...
[ApiController,Route("api/checklists"),Authorize]
public sealed class ChecklistsController(IMobileService service):ControllerBase
{
    ...
    [HttpPost("templates")] public async Task<IActionResult> Template(ChecklistTemplateCommand command,CancellationToken ct)=>Created("api/checklists/templates",new{id=await service.AddTemplateAsync(command,ct)});
    [HttpPost("apply")] public async Task<IActionResult> Apply(ChecklistApplyCommand command,CancellationToken ct)=>Created("api/checklists",new{id=await service.ApplyChecklistAsync(command,ct)});
    [HttpPost("{id:guid}/complete")] public async Task<IActionResult> Complete(Guid id,ChecklistCompleteCommand command,CancellationToken ct){await service.CompleteChecklistAsync(id,command,ct);return NoContent();}
}
