...
public sealed class OperationsController(IOperationsService service) : ControllerBase
{
    ...
    [HttpPost("api/fleet/assets"), Authorize(Policy=Permissions.FleetWrite)] public async Task<IActionResult> Asset(FleetAssetCommand x,CancellationToken ct){var id=await service.CreateAssetAsync(x,ct);return Created("/api/fleet/assets/"+id,new{id});}
    [HttpPut("api/fleet/assets/{id:guid}"), Authorize(Policy=Permissions.FleetWrite)] public async Task<IActionResult> Asset(Guid id,FleetAssetCommand x,CancellationToken ct){await service.UpdateAssetAsync(id,x,ct);return NoContent();}
