...
public sealed class OperationsController(IOperationsService service) : ControllerBase
{
    ...
    [HttpPost("api/purchases/{id:guid}/{action:regex(^(submit|approve|cancel)$)}"), Authorize(Policy=Permissions.PurchasingApprove)] public async Task<IActionResult> PurchaseAction(Guid id,string action,CancellationToken ct){await service.ChangePurchaseStatusAsync(id,action,ct);return NoContent();}
    ...
    [HttpGet("api/fleet/assets"), Authorize(Policy=Permissions.FleetRead)] public Task<IReadOnlyList<dynamic>> Assets(CancellationToken ct)=>service.ListAssetsAsync(ct);
    [HttpPost("api/fleet/assets"), Authorize(Policy=Permissions.FleetWrite)] public async Task<IActionResult> Asset(OperationalFleetAssetCommand x,CancellationToken ct){var id=await service.CreateAssetAsync(x,ct);return Created("/api/fleet/assets/"+id,new{id});}
    [HttpPut("api/fleet/assets/{id:guid}"), Authorize(Policy=Permissions.FleetWrite)] public async Task<IActionResult> Asset(Guid id,OperationalFleetAssetCommand x,CancellationToken ct){await service.UpdateAssetAsync(id,x,ct);return NoContent();}
