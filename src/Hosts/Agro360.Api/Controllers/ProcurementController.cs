...
public sealed class ProcurementController(IProcurementService service) : ControllerBase
{
    ...
    [HttpPost("receipts/{id:guid}/quality"), Authorize(Policy = Permissions.PurchasingInspect)] public async Task<IActionResult> Quality(Guid id, ReceiptQualityCommand x, CancellationToken ct) { await service.DecideReceiptQualityAsync(id, x, ct); return NoContent(); }
    [HttpPost("receipts/{id:guid}/cancel"), Authorize(Policy = Permissions.PurchasingCancelReceipt)] public async Task<IActionResult> CancelReceipt(Guid id, CancelReceiptCommand x, CancellationToken ct) { await service.CancelReceiptAsync(id, x, ct); return NoContent(); }
