...
public sealed class ProcurementController(IProcurementService service) : ControllerBase
{
 ...
 [HttpPost("suppliers"),Authorize(Policy=Permissions.PurchasingWrite)] public async Task<IActionResult> Supplier(SupplierCommand x,CancellationToken ct)=>Created("api/procurement/suppliers",new{id=await service.SaveSupplierAsync(null,x,ct)});
 [HttpPut("suppliers/{id:guid}"),Authorize(Policy=Permissions.PurchasingWrite)] public async Task<IActionResult> Supplier(Guid id,SupplierCommand x,CancellationToken ct){await service.SaveSupplierAsync(id,x,ct);return NoContent();}
 ...
 [HttpPost("receipts"),Authorize(Policy=Permissions.PurchasingReceive)] public async Task<IActionResult> Receive(ReceiptCommand x,CancellationToken ct)=>Created("api/procurement/receipts",new{id=await service.ReceiveAsync(x,ct)});
