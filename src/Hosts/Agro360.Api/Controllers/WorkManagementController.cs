...
public sealed class WorkManagementController(IWorkManagementService service) : ControllerBase
{
    ...
    [HttpPost("api/operation-center/{key}/assign"), Authorize(Policy = Permissions.WorkWrite)] public async Task<IActionResult> AssignOccurrence(string key, [FromBody] WorkLookup responsible, CancellationToken ct) { await service.AssignOccurrenceAsync(key, responsible.Id, ct); return NoContent(); }
}
