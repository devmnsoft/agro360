...
public sealed class WorkManagementController(IWorkManagementService service) : ControllerBase
{
    ...
    [HttpPost("api/alerts/{id:guid}/read"), Authorize(Policy = Permissions.WorkWrite)] public async Task<IActionResult> ReadAlert(Guid id, CancellationToken ct) { await service.ChangeAlertAsync(id, "read", ct); return NoContent(); }
    [HttpPost("api/alerts/{id:guid}/resolve"), Authorize(Policy = Permissions.WorkWrite)] public async Task<IActionResult> ResolveAlert(Guid id, CancellationToken ct) { await service.ChangeAlertAsync(id, "resolve", ct); return NoContent(); }
    ...
    [HttpGet("api/operation-center"), Authorize(Policy = Permissions.WorkRead)] public Task<PagedResult<OperationOccurrenceRow>> OperationCenter([FromQuery] OperationOccurrenceQuery query, CancellationToken ct) => service.OperationCenterAsync(query, ct);
    [HttpPost("api/operation-center/{key}/view"), Authorize(Policy = Permissions.WorkRead)] public async Task<IActionResult> ViewOccurrence(string key, CancellationToken ct) { await service.MarkOccurrenceViewedAsync(key, ct); return NoContent(); }
    [HttpGet("api/operation-center/{key}/eligible-users"), Authorize(Policy = Permissions.WorkWrite)] public Task<PagedResult<EligibleOccurrenceUser>> EligibleUsers(string key, string? search, int page = 1, int pageSize = 20, CancellationToken ct = default) => service.EligibleOccurrenceUsersAsync(key, search, page, pageSize, ct);
    [HttpPut("api/operation-center/{key}/assignment"), Authorize(Policy = Permissions.WorkWrite)] public async Task<IActionResult> AssignOccurrence(string key, [FromBody] OccurrenceAssignmentCommand command, CancellationToken ct) { await service.AssignOccurrenceAsync(key, command, ct); return NoContent(); }
}
