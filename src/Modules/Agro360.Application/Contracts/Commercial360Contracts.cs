...
public sealed record OpportunityCommand([Required]Guid CustomerId,[Required,MaxLength(180)]string Name,[Range(0,double.MaxValue)]decimal EstimatedValue,[Required]string Stage,[Range(0,100)]int Probability,Guid? ProductId,Guid? RepresentativeId,DateOnly? ExpectedClose,string? Source,string? NextAction,string? LossReason);
...
public sealed record OrderItemCommand([Required]Guid ProductId,Guid? LotId,[Range(0.000001,double.MaxValue)]decimal Quantity,[Required]string Unit,[Range(0,double.MaxValue)]decimal UnitPrice,[Range(0,100)]decimal DiscountPercentage,[Range(0,100)]decimal MaximumDiscount);
