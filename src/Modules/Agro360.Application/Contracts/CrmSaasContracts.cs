...
public sealed record OpportunityCommand([Required]Guid LeadId,[Required,MaxLength(180)]string Title,[Range(0,double.MaxValue)]decimal RevenuePotential,[Range(0,100)]decimal Probability,Guid? OwnerId,DateOnly? ExpectedCloseOn);
...
public interface ICrmSaasService
{
 ...
 Task<IReadOnlyList<CrmOpportunity>> OpportunitiesAsync(CancellationToken ct); Task<Guid> CreateOpportunityAsync(OpportunityCommand command,CancellationToken ct); Task<Guid?> TransitionOpportunityAsync(Guid id,OpportunityTransition command,CancellationToken ct);
