...
public sealed record MarketplaceModule(Guid Id, string Code, string Name, string Description, string[] Segments, string[] Plans, string[] Dependencies, bool Official, bool Featured, string Status, Guid? PendingRequestId);
...
public sealed record ModuleRequestSummary(Guid Id, Guid TenantId, string TenantName, Guid ModuleId, string ModuleName, string ModuleCode, string Status, string? Justification, string OfferSnapshot, DateTimeOffset CreatedAt, DateTimeOffset? DecidedAt, string? DecisionReason);
