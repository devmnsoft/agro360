...
public sealed record IntegrationFiscalDocumentCommand([Required,RegularExpression("^(NFE|CTE|MDFE|PRODUCER_INVOICE|GTA|SANITARY_CERTIFICATE|REPORT)$")]string Type,[Required,MaxLength(240)]string FileName,[Required]string ContentBase64,Guid? PurchaseId,Guid? SaleId,Guid? ShipmentId,Guid? LotId,Guid? ProducerId,Guid? SupplierId);
...
public sealed record SplitCommand([Required]Guid SaleId,[Range(0.01,double.MaxValue)]decimal GrossAmount,[Required,MinLength(1)]PaymentSplitParticipant[] Participants);
public sealed record PaymentSplitParticipant([Required]Guid PartyId,[Range(0.01,100)]decimal Percentage);
...
public interface IIntegrationService
{
 ...
 Task<Guid> AddFiscalDocumentAsync(IntegrationFiscalDocumentCommand command,CancellationToken ct); Task<object> FiscalDocumentsAsync(CancellationToken ct);
