...
public sealed record SupplierCommand(string LegalName,string? TradeName,string? TaxDocument,string? StateRegistration,string Type,string Category,string? Email,string? Phone,string? Address,string? City,string? State,string Country,string? MainContact,string? PaymentTerms,int AverageDeliveryDays,string Status,string? RejectionReason,string? Notes,string[] Tags);
...
public sealed record ReceiptCommand(Guid PurchaseOrderId,DateTimeOffset ReceivedAt,string? InvoiceDocument,bool OverrideExcess,string? ExcessJustification,IReadOnlyList<ReceiptLineCommand> Items);
...
{
 Task<dynamic> DashboardAsync(CancellationToken ct); Task<IReadOnlyList<dynamic>> SuppliersAsync(ProcurementQuery q,CancellationToken ct); Task<Guid> SaveSupplierAsync(Guid? id,SupplierCommand x,CancellationToken ct); Task HomologateAsync(Guid supplierId,bool approve,HomologationCommand x,CancellationToken ct);
 ...
 Task<Guid> CreateOrderAsync(PurchaseOrderCommand x,CancellationToken ct); Task<IReadOnlyList<dynamic>> OrdersAsync(ProcurementQuery q,CancellationToken ct); Task ApproveOrderAsync(Guid id,string? comment,CancellationToken ct); Task<Guid> ReceiveAsync(ReceiptCommand x,CancellationToken ct); Task<byte[]> ExportAsync(string report,ProcurementQuery q,CancellationToken ct);
}
