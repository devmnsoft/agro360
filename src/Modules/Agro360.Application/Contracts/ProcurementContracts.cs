...
public interface IProcurementService
{
    ...
    Task<Guid> CreateOrderAsync(PurchaseOrderCommand x, CancellationToken ct); Task<IReadOnlyList<dynamic>> OrdersAsync(ProcurementQuery q, CancellationToken ct); Task ApproveOrderAsync(Guid id, string? comment, CancellationToken ct); Task<Guid> ReceiveAsync(ProcurementReceiptCommand x, CancellationToken ct); Task<IReadOnlyList<dynamic>> ReceiptsAsync(ProcurementQuery q, CancellationToken ct); Task<IReadOnlyList<dynamic>> PendingOrderItemsAsync(Guid orderId, CancellationToken ct); Task<dynamic> ReceiptOptionsAsync(CancellationToken ct); Task<byte[]> ExportAsync(string report, ProcurementQuery q, CancellationToken ct);
}
