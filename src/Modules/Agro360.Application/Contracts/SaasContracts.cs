...
public sealed class BillingChargeSummary { public Guid Id { get; init; } public Guid TenantId { get; init; } public string TenantName { get; init; } = string.Empty; public string PlanName { get; init; } = string.Empty; public DateOnly Competence { get; init; } public DateOnly DueOn { get; init; } public decimal BaseAmount { get; init; } public decimal AdditionalAmount { get; init; } public decimal Discount { get; init; } public decimal Amount { get; init; } public decimal PaidAmount { get; init; } public decimal OutstandingAmount { get; init; } public string[] ModuleCodes { get; init; } = []; public string Status { get; init; } = string.Empty; public string? Notes { get; init; } public DateOnly? PaidOn { get; init; } }
public sealed record BillingChargeCommand(Guid TenantId, Guid SubscriptionId, DateOnly Competence, DateOnly DueOn, decimal Amount, string? Notes, decimal AdditionalAmount = 0, decimal Discount = 0, string[]? ModuleCodes = null);
public sealed record BillingStatusCommand(string Status, string Reason, DateOnly? PaidOn = null, decimal? PaidAmount = null, string? Observation = null);
public sealed record ManualPaymentCommand(string IdempotencyKey, string PaymentReference, decimal Amount, DateOnly PaidOn, string Observation);
public sealed record ManualPaymentResult(Guid PaymentId, string ChargeStatus, decimal AppliedAmount, decimal OutstandingAmount, decimal CreditAmount, bool Replayed);
