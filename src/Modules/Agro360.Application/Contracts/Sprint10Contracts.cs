...
public sealed record PartnerCommand(string Name,string Type,string? Document,string? Contact,bool Active);
...
public sealed record SplitParticipant(Guid PartnerId,string Role,decimal Value,bool Percentage);
public sealed record SplitCalculationCommand(Guid SaleId,decimal GrossAmount,IReadOnlyList<SplitParticipant> Participants);
...
public interface ISprint10Service
{
 ...
 Task<IReadOnlyList<dynamic>> PartnersAsync(CancellationToken ct); Task<Guid> AddPartnerAsync(PartnerCommand command,CancellationToken ct); Task<Guid> AddCommissionRuleAsync(CommissionRuleCommand command,CancellationToken ct); Task<dynamic> CalculateSplitAsync(SplitCalculationCommand command,CancellationToken ct); Task ApproveSplitAsync(Guid id,CancellationToken ct); Task<dynamic> DashboardAsync(CancellationToken ct);
}
