...
public interface ISupportCustomerSuccessService
{
 ...
 Task<IReadOnlyList<KnowledgeArticle>> ArticlesAsync(string? search,string? moduleCode,bool publicOnly,CancellationToken ct); Task<Guid> CreateArticleAsync(ArticleCommand command,CancellationToken ct); Task PublishArticleAsync(Guid id,bool archive,CancellationToken ct); Task RateArticleAsync(Guid id,ArticleFeedbackCommand command,CancellationToken ct);
