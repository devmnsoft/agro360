...
public static class PortalRules
{
    ...
    public static string Email(string value) { try { var candidate = value.Trim(); var email = new MailAddress(candidate).Address; return string.Equals(email, candidate, StringComparison.OrdinalIgnoreCase) ? email.ToLowerInvariant() : throw new FormatException(); } catch { throw new DomainException("Informe um e-mail válido.", "portal.email"); } }
