...
public sealed class LookupService(DatabaseExecutor database, ITenantContext tenant) : ILookupService
{
    ...
    {
        ["properties"] = "select id,name label,concat_ws(' · ',municipality,state) description,'ACTIVE' status,'{}'::jsonb metadata from geo.farms where tenant_id=@TenantId and deleted_at is null",
        ["production-areas"] = "select id,name label,area_ha||' ha' description,'ACTIVE' status,jsonb_build_object('farmId',farm_id,'areaHa',area_ha) metadata from geo.fields where tenant_id=@TenantId and deleted_at is null",
        ["fields"] = "select f.id,f.name label,concat(f.area_ha,' ha · ',p.name) description,'ACTIVE' status,jsonb_build_object('propertyId',f.farm_id,'areaHa',f.area_ha) metadata from geo.fields f join geo.farms p on p.id=f.farm_id and p.tenant_id=f.tenant_id where f.tenant_id=@TenantId and f.deleted_at is null",
        ...
        ["cost-centers"] = "select id,name label,concat(code,' · ',kind) description,case when active then 'ACTIVE' else 'INACTIVE' end status,'{}'::jsonb metadata from finance.cost_centers where tenant_id=@TenantId",
        ["livestock-batches"] = "select id,name label,concat(species,' · ',category) description,case when active then 'ACTIVE' else 'INACTIVE' end status,jsonb_build_object('headCount',head_count) metadata from livestock.herds where tenant_id=@TenantId",
        ["storage-lots"] = "select id,code label,concat(current_balance,' disponível') description,status,jsonb_build_object('productId',product_id,'balance',current_balance) metadata from storage.lots where tenant_id=@TenantId",
...
public sealed class Agriculture360Service(DatabaseExecutor database, ITenantContext tenant) : IAgriculture360Service
{
    ...
    public Task<PagedResult<AgricultureRecord>> ListAsync(string module, int page, int pageSize, CancellationToken cancellationToken)
    {
        ...
        {
            using var grid = await c.QueryMultipleAsync(new CommandDefinition("select count(*) from agriculture.records where tenant_id=@TenantId and module=@Module and deleted_at is null; select id,module,status,created_at CreatedAt,data::text Data from agriculture.records where tenant_id=@TenantId and module=@Module and deleted_at is null order by created_at desc limit @PageSize offset @Offset", new { tenant.TenantId, Module = module, PageSize = pageSize, Offset = (page - 1) * pageSize }, t, cancellationToken: cancellationToken));
    ...
    public Task<AgricultureRecord> CreateAsync(string module, AgricultureCommand command, CancellationToken cancellationToken)
    {
        ...
        return database.InTenantTransactionAsync(async (c, t) =>
        {
            ...
            await c.ExecuteAsync(new CommandDefinition("insert into agriculture.records(id,tenant_id,module,status,data,created_by) values(@Id,@TenantId,@Module,@Status,cast(@Data as jsonb),@UserId)", new { Id=id, tenant.TenantId, Module=module, Status=status, Data=JsonSerializer.Serialize(command, JsonOptions), tenant.UserId }, t, cancellationToken:cancellationToken));
    ...
    {
        EnsureModule(module); var next=action.ToLowerInvariant() switch {"approve"=>"APPROVED","revise"=>"REVISION","start"=>"IN_PROGRESS","pause"=>"PAUSED","complete"=>"COMPLETED","cancel"=>"CANCELLED",_=>throw new DomainException("Transição inválida.", "agriculture.transition_invalid")};
        if(next=="COMPLETED" && module=="work-orders" && (command?.ResponsibleId is null || command.ChecklistRequired && !command.ChecklistCompleted)) throw new DomainException("Responsável e checklist obrigatório concluído são necessários.","agriculture.work_order_incomplete");
        if(next=="CANCELLED" && string.IsNullOrWhiteSpace(command?.CancellationReason)) throw new DomainException("Informe o motivo do cancelamento.","agriculture.cancellation_reason_required");
        return database.InTenantTransactionAsync(async(c,t)=>{var row=await c.QuerySingleOrDefaultAsync<RecordRow>(new CommandDefinition("update agriculture.records set status=@Status,updated_at=now(),updated_by=@UserId,version=version+1 where id=@Id and tenant_id=@TenantId and module=@Module and deleted_at is null returning id,module,status,created_at CreatedAt,data::text Data",new{Status=next,tenant.UserId,Id=id,tenant.TenantId,Module=module},t,cancellationToken:cancellationToken)); if(row is null) throw new NotFoundException("Registro agrícola", id); await c.ExecuteAsync(new CommandDefinition("insert into agriculture.status_history(id,tenant_id,record_id,from_status,to_status,reason,changed_by) select gen_random_uuid(),tenant_id,id,status,@Status,@Reason,@UserId from agriculture.records where id=@Id and tenant_id=@TenantId",new{Status=next,Reason=command?.CancellationReason,tenant.UserId,Id=id,tenant.TenantId},t,cancellationToken:cancellationToken)); return Map(row);},cancellationToken);
    }

    public Task<AgricultureDashboard> DashboardAsync(CancellationToken cancellationToken) => database.InTenantTransactionAsync(async(c,t)=> await c.QuerySingleAsync<AgricultureDashboard>(new CommandDefinition("select (select count(*) from agriculture.seasons where tenant_id=@TenantId and status=2) ActiveSeasons,coalesce((select sum(planned_area_ha) from agriculture.seasons where tenant_id=@TenantId and status=2),0) PlantedArea,count(*) filter(where module='field-notes' and status='PLANNED') PlannedActivities,count(*) filter(where module='field-notes' and status='PLANNED' and (data->>'plannedAt')::timestamptz<now()) OverdueActivities,count(*) filter(where module='field-notes' and status='COMPLETED' and updated_at>=date_trunc('month',now())) CompletedThisMonth,coalesce(sum((data->>'estimatedCost')::numeric),0) PlannedCost,coalesce(sum(case when status='COMPLETED' then (data->>'estimatedCost')::numeric else 0 end),0) ActualCost,case when coalesce(sum((data->>'area')::numeric),0)>0 then coalesce(sum(case when status='COMPLETED' then (data->>'estimatedCost')::numeric else 0 end),0)/sum((data->>'area')::numeric) else 0 end CostPerHectare,count(*) filter(where module='scouting' and status not in('CLOSED','CANCELLED')) OpenOccurrences,count(*) filter(where module='scouting' and data->>'severity'='CRITICAL') CriticalOccurrences,coalesce(sum(case when module='plans' then (data->>'quantity')::numeric else 0 end),0) PlannedInputs,coalesce(sum(case when module='applications' then (data->>'quantity')::numeric else 0 end),0) ActualInputs,count(*) filter(where module='applications' and status='COMPLETED') Applications,count(*) filter(where module='irrigations' and status='COMPLETED') Irrigations,coalesce((select sum(expected_yield_per_ha) from agriculture.seasons where tenant_id=@TenantId),0) ExpectedYield,0::numeric ActualYield,array_remove(array[case when count(*) filter(where module='scouting' and data->>'severity'='CRITICAL')>0 then 'Ocorrência fitossanitária crítica' end,case when count(*) filter(where module='field-notes' and status='PLANNED' and (data->>'plannedAt')::timestamptz<now())>0 then 'Atividades agrícolas atrasadas' end],null) Alerts from agriculture.records where tenant_id=@TenantId and deleted_at is null",new{tenant.TenantId},t,cancellationToken:cancellationToken)),cancellationToken);

    private static void Validate(string module,AgricultureCommand x){ Agriculture360Rules.Period(x.StartedAt,x.FinishedAt); if(module is "field-notes" or "plans" or "scouting" or "recommendations" or "applications" or "irrigations" or "work-orders") Agriculture360Rules.Required(x.PropertyId,"Propriedade"); if(module=="field-notes") Agriculture360Rules.Required(x.CropSeasonId,"Safra"); if(module is "field-notes" or "scouting" or "applications" or "irrigations" or "work-orders") Agriculture360Rules.Required(x.FieldId,"Talhão"); if(module=="plans"){Agriculture360Rules.Required(x.CropSeasonId,"Safra");Agriculture360Rules.Required(x.CropId,"Cultura");Agriculture360Rules.UniqueFields(x.FieldIds);} if(module=="applications"){Agriculture360Rules.Required(x.InventoryItemId,"Produto");Agriculture360Rules.Positive(x.Dose,"Dose");} if(module=="irrigations") Agriculture360Rules.Positive(x.Area,"Área"); if(module=="weather-records"){Agriculture360Rules.Required(x.PropertyId,"Propriedade");Agriculture360Rules.Weather(x.Rainfall,x.Temperature,x.Humidity,x.Wind);} if(module=="work-orders"&&x.Status=="COMPLETED") Agriculture360Rules.Required(x.ResponsibleId,"Responsável"); if(module=="scouting"&&!new[]{"LOW","MEDIUM","HIGH","CRITICAL"}.Contains(x.Severity)) throw new DomainException("Selecione uma severidade válida.", "agriculture.severity_invalid"); }
    private static string NormalizeStatus(string? status,string module)
    {
        ...
        if(!new[]{"OPEN","PLANNED","RELEASED","IN_PROGRESS","PAUSED","COMPLETED","CANCELLED","APPROVED","REVISION","CLOSED"}.Contains(value)) throw new DomainException("Status inválido.","agriculture.status_invalid");
        return value;
    }
    ...
    private async Task ValidateReferences(System.Data.IDbConnection c,System.Data.IDbTransaction t,AgricultureCommand x,CancellationToken ct){var valid=await c.ExecuteScalarAsync<bool>(new CommandDefinition("select (@PropertyId is null or exists(select 1 from geo.farms where id=@PropertyId and tenant_id=@TenantId and deleted_at is null)) and (@FieldId is null or exists(select 1 from geo.fields where id=@FieldId and tenant_id=@TenantId and deleted_at is null)) and (@ResponsibleId is null or exists(select 1 from identity.users where id=@ResponsibleId and tenant_id=@TenantId and status='ACTIVE')) and (@InventoryItemId is null or exists(select 1 from inventory.products where id=@InventoryItemId and tenant_id=@TenantId and deleted_at is null)) and (@CropSeasonId is null or exists(select 1 from agriculture.seasons where id=@CropSeasonId and tenant_id=@TenantId and deleted_at is null)) and (@CropId is null or exists(select 1 from agriculture.crops where id=@CropId and tenant_id=@TenantId and active)) and (@MachineId is null or exists(select 1 from fleet.assets where id=@MachineId and tenant_id=@TenantId and status not in ('SOLD','INACTIVE'))) and (@CostCenterId is null or exists(select 1 from finance.cost_centers where id=@CostCenterId and tenant_id=@TenantId and active))",new{x.PropertyId,x.FieldId,x.ResponsibleId,x.InventoryItemId,x.CropSeasonId,x.CropId,x.MachineId,x.CostCenterId,tenant.TenantId},t,cancellationToken:ct));if(!valid)throw new DomainException("Uma seleção não pertence ao tenant ou está inativa.","agriculture.lookup_invalid");}
    private async Task ApplyEffects(System.Data.IDbConnection c,System.Data.IDbTransaction t,string module,Guid id,AgricultureCommand x,string status,CancellationToken ct){if(module is not ("applications" or "field-notes")||status!="COMPLETED"||x.InventoryItemId is null||x.Quantity is null)return;Agriculture360Rules.Positive(x.Quantity,"Quantidade");var changed=await c.ExecuteAsync(new CommandDefinition("update inventory.stock_balances set available=available-@Quantity,version=version+1,updated_at=now() where tenant_id=@TenantId and product_id=@ProductId and available-reserved>=@Quantity",new{x.Quantity,tenant.TenantId,ProductId=x.InventoryItemId},t,cancellationToken:ct));if(changed==0)throw new ConflictException("Estoque insuficiente ou produto vencido para a aplicação.","agriculture.stock_unavailable");}
