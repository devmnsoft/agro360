...
public sealed class Commercial360Service(DatabaseExecutor db, ITenantContext tenant) : ICommercial360Service
{
    ...
    public Task<Guid> CreateOrderAsync(SalesOrderCommand command, CancellationToken ct) => db.InTenantTransactionAsync(async (c, t) =>
    {
        ...
        var policyRows = (await c.QueryAsync<(Guid ProductId, string Unit, decimal MaximumDiscount)>(
            "select i.product_id ProductId,i.unit,i.maximum_discount MaximumDiscount from agro360.sales_price_tables p join agro360.sales_price_table_items i on i.tenant_id=p.tenant_id and i.price_table_id=p.id where p.tenant_id=@TenantId and p.segment_id=@SegmentId and p.status='ACTIVE' and p.deleted_at is null and current_date between p.valid_from and p.valid_to and i.product_id=any(@ProductIds) order by p.is_default desc,p.valid_from desc",
        ...
        var policies = policyRows
            ...
            .ToDictionary(x => x.Key, x => x.First().MaximumDiscount);
        var pricedItems = command.Items.Select(item =>
        {
            if (!policies.TryGetValue((item.ProductId, item.Unit.Trim().ToUpperInvariant()), out var maximumDiscount))
            ...
            return (item.Quantity, item.UnitPrice, item.DiscountPercentage, maximumDiscount);
        }).ToArray();
        var total = CommercialRules.ValidateOrder(pricedItems);
        var id = Guid.CreateVersion7();
        await c.ExecuteAsync("insert into agro360.sales_orders(id,tenant_id,customer_id,representative_id,property_id,contract_id,order_number,status,total_amount,freight,payment_terms,expected_delivery,notes,created_by,updated_by) values(@Id,@TenantId,@CustomerId,@RepresentativeId,@PropertyId,@ContractId,'PED-'||nextval('agro360.sales_order_number_seq'),'DRAFT',@Total,@Freight,@PaymentTerms,@ExpectedDelivery,@Notes,@UserId,@UserId)", new { Id = id, tenant.TenantId, command.CustomerId, command.RepresentativeId, command.PropertyId, command.ContractId, Total = total + command.Freight, command.Freight, command.PaymentTerms, command.ExpectedDelivery, command.Notes, tenant.UserId }, t);
        foreach (var item in command.Items)
            await c.ExecuteAsync("insert into agro360.sales_order_items(id,tenant_id,order_id,product_id,lot_id,quantity,unit,unit_price,discount_percentage,total_amount) values(gen_random_uuid(),@TenantId,@OrderId,@ProductId,@LotId,@Quantity,@Unit,@UnitPrice,@DiscountPercentage,round(@Quantity*@UnitPrice*(1-@DiscountPercentage/100),2))", new { tenant.TenantId, OrderId = id, item.ProductId, item.LotId, item.Quantity, Unit = item.Unit.Trim().ToUpperInvariant(), item.UnitPrice, item.DiscountPercentage }, t);
        return id;
    }, ct);
