...
public sealed class DataGovernanceService(DatabaseExecutor db, ITenantContext tenant) : IDataGovernanceService
{
    public Task<ImportBatch> CreateImportAsync(ImportBatchCommand command, CancellationToken ct) => db.InTenantTransactionAsync(async (connection, transaction) =>
    {
        ...
        await connection.ExecuteAsync("""
            insert into agro360.governance_data_import_batches(id,tenant_id,module,file_name,status,total_rows,valid_rows,error_rows,confirmed_critical,created_by,updated_by)
        ...
        foreach (var error in errors)
            await connection.ExecuteAsync("insert into agro360.governance_data_import_errors(id,tenant_id,batch_id,row_number,column_name,error_code,message,severity,created_by) values(gen_random_uuid(),@TenantId,@Id,@RowNumber,@ColumnName,@Code,@Message,@Severity,@UserId)", new { tenant.TenantId, tenant.UserId, id, error.RowNumber, error.ColumnName, error.Code, error.Message, error.Severity }, transaction);
    ...
    public Task<PagedResult<ImportBatch>> ImportsAsync(int page, int pageSize, CancellationToken ct) => db.InTenantTransactionAsync(async (c, t) =>
    {
        ...
        var total = await c.ExecuteScalarAsync<int>("select count(*) from agro360.governance_data_import_batches where tenant_id=@TenantId and deleted_at is null", new { tenant.TenantId }, t);
        var items = (await c.QueryAsync<ImportBatch>("select id,module,file_name filename,status,total_rows totalrows,valid_rows validrows,error_rows errorrows,created_at createdat from agro360.governance_data_import_batches where tenant_id=@TenantId and deleted_at is null order by created_at desc limit @PageSize offset @Offset", new { tenant.TenantId, PageSize = pageSize, Offset = (page - 1) * pageSize }, t)).AsList();
