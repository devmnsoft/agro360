...
public sealed class ExecutiveIntelligenceService(DatabaseExecutor database, ITenantContext tenant, ILogger<ExecutiveIntelligenceService> logger) : IExecutiveIntelligenceService
{
    ...
    {
        var indicators = (await c.QueryAsync<ExecutiveKpi>(new CommandDefinition("""select d.id,d.code,d.name,d.category,d.unit,s.calculated_value value,d.target,s.status,s.calculated_at,s.calculation_error from agro360.intelligence_kpi_definitions d left join lateral(select calculated_value,status,calculated_at,calculation_error from agro360.intelligence_kpi_snapshots where tenant_id=@TenantId and kpi_id=d.id order by reference_date desc,created_at desc limit 1)s on true where d.tenant_id=@TenantId and d.active and d.deleted_at is null order by d.category,d.name""", new { tenant.TenantId }, t, cancellationToken: ct))).ToArray();
        var totals = await c.QuerySingleAsync(new CommandDefinition("""select (select count(*) from agro360.intelligence_alerts where tenant_id=@TenantId and status not in('RESOLVED','CANCELLED')) open_alerts,(select count(*) from agro360.intelligence_alerts where tenant_id=@TenantId and severity='CRITICAL' and status not in('RESOLVED','CANCELLED')) critical_alerts,(select count(*) from agro360.intelligence_risks where tenant_id=@TenantId and status='OPEN') open_risks,(select count(*) from agro360.intelligence_recommendations where tenant_id=@TenantId and status='NEW') pending_recommendations""", new { tenant.TenantId }, t, cancellationToken: ct));
    ...
    {
        var source=await c.ExecuteScalarAsync<string?>(new CommandDefinition("select data_source from agro360.intelligence_kpi_definitions where id=@Id and tenant_id=@TenantId and active and deleted_at is null",new{Id=id,tenant.TenantId},t,cancellationToken:ct));
    ...
    {
        var severity=await c.ExecuteScalarAsync<string?>(new CommandDefinition("select severity from agro360.intelligence_alerts where id=@Id and tenant_id=@TenantId",new{Id=id,tenant.TenantId},t,cancellationToken:ct))??throw new KeyNotFoundException("Alerta não encontrado.");
        ...
        await c.ExecuteAsync(new CommandDefinition("update agro360.intelligence_alerts set status=@Status,responsible_id=@ResponsibleId,resolved_at=case when @Status='RESOLVED' then now() else resolved_at end,resolved_by=case when @Status='RESOLVED' then @UserId else resolved_by end,updated_at=now(),updated_by=@UserId where id=@Id and tenant_id=@TenantId;insert into agro360.intelligence_alert_events(id,tenant_id,alert_id,event_type,comment,created_by,updated_by) values(gen_random_uuid(),@TenantId,@Id,@Status,@Comment,@UserId,@UserId)",new{Id=id,tenant.TenantId,x.Status,x.ResponsibleId,x.Comment,UserId=userId},t,cancellationToken:ct));
    },ct));
    ...
    { var severity=await c.ExecuteScalarAsync<string?>(new CommandDefinition("select severity from agro360.intelligence_recommendations where id=@Id and tenant_id=@TenantId",new{Id=id,tenant.TenantId},t,cancellationToken:ct))??throw new KeyNotFoundException("Recomendação não encontrada.");if(x.Status=="REJECTED"&&(severity=="HIGH"||severity=="CRITICAL")&&string.IsNullOrWhiteSpace(x.Reason))throw new ArgumentException("Rejeição de recomendação alta ou crítica exige motivo.");await c.ExecuteAsync(new CommandDefinition("update agro360.intelligence_recommendations set status=@Status,decision_reason=@Reason,updated_at=now(),updated_by=@UserId where id=@Id and tenant_id=@TenantId;insert into agro360.intelligence_recommendation_events(id,tenant_id,recommendation_id,event_type,reason,created_by,updated_by) values(gen_random_uuid(),@TenantId,@Id,@Status,@Reason,@UserId,@UserId)",new{Id=id,tenant.TenantId,x.Status,x.Reason,UserId=userId},t,cancellationToken:ct));},ct));

    public Task<byte[]> ExportAsync(string report,IntelligencePageFilter f,Guid userId,CancellationToken ct)=>Guard("export",()=>database.InTenantTransactionAsync(async(c,t)=>{var allowed=new HashSet<string>(StringComparer.OrdinalIgnoreCase){"indicators","snapshots","alerts","risks","recommendations","audit"};if(!allowed.Contains(report))throw new ArgumentException("Relatório não suportado.");var rows=await c.QueryAsync<(string Type,string Status,string Description,DateTimeOffset CreatedAt)>(new CommandDefinition("select 'ALERT' type,status,description,created_at from agro360.intelligence_alerts where tenant_id=@TenantId and (@Status is null or status=@Status) order by created_at desc limit @PageSize",new{tenant.TenantId,Status=string.IsNullOrWhiteSpace(f.Status)?null:f.Status,PageSize=Math.Clamp(f.PageSize,1,100)},t,cancellationToken:ct));var csv=new StringBuilder("Tipo;Status;Descrição;Data\r\n");foreach(var row in rows)csv.AppendLine($"{Csv(row.Type)};{Csv(row.Status)};{Csv(row.Description)};{row.CreatedAt:O}");await c.ExecuteAsync("insert into agro360.intelligence_report_exports(id,tenant_id,report,filters,row_count,created_by) values(gen_random_uuid(),@TenantId,@Report,'{}',@Count,@UserId)",new{tenant.TenantId,Report=report,Count=rows.Count(),UserId=userId},t);return Encoding.UTF8.GetPreamble().Concat(Encoding.UTF8.GetBytes(csv.ToString())).ToArray();},ct));
