...
public sealed class FieldOperationsService(DatabaseExecutor database, ITenantContext tenant) : IFieldOperationsService
{
    ...
    public Task ApplyMaterialEventAsync(Guid orderId, Guid materialId, FieldMaterialEventCommand command, CancellationToken cancellationToken) => database.InTenantTransactionAsync(async (c, t) =>
    {
        ...
        var kind = command.EventType.Trim().ToUpperInvariant(); if (kind is not ("RESERVE" or "DELIVER" or "CONSUME" or "RETURN" or "LOSS" or "RELEASE")) throw new DomainException("Evento de material inválido.");
        if (kind == "LOSS" && string.IsNullOrWhiteSpace(command.Reason)) throw new DomainException("Informe o motivo e responsável pela perda.");
        ...
        if (await c.ExecuteScalarAsync<bool>(new CommandDefinition("select exists(select 1 from agro360.field_material_events where tenant_id=@TenantId and idempotency_key=@Key)", new { tenant.TenantId, Key = command.IdempotencyKey }, t, cancellationToken: cancellationToken))) return;
        ...
        Guid? movement = null;
        if (kind is "CONSUME" or "LOSS" or "RETURN") { if (m.WarehouseId is null) throw new ConflictException("Defina o depósito antes de movimentar estoque."); var qty = kind == "RETURN" ? command.Quantity : -command.Quantity; movement = await c.ExecuteScalarAsync<Guid>(new CommandDefinition("select agro360.inventory_apply_stock_movement(@TenantId,@WarehouseId,@ProductId,@Quantity,coalesce(@UnitCost,0),@Type,@Reference,null,null,@UserId,@Reason)", new { tenant.TenantId, m.WarehouseId, m.ProductId, Quantity = qty, m.UnitCost, Type = kind == "RETURN" ? "ENTRY" : "EXIT", Reference = orderId, tenant.UserId, command.Reason }, t, cancellationToken: cancellationToken)); }
