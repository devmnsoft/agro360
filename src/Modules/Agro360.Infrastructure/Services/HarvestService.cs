...
public sealed class HarvestService(DatabaseExecutor database, ITenantContext tenant) : IHarvestService
{
    ...
    public Task<SeasonClosingDto> GetClosingAsync(Guid seasonId, DateOnly cutoffDate, CancellationToken cancellationToken) => database.InTenantTransactionAsync(async (db, tx) =>
    {
        ...
        var versions = (await db.QueryAsync<ClosingVersionRow>(new CommandDefinition("select id,version,state,cutoff_date CutoffDate,generated_at GeneratedAt,responsible_id ResponsibleId,supersedes_id SupersedesId,reason,notes,indicators::text Indicators,issues::text Issues from agro360.harvest_closing_versions where tenant_id=@TenantId and season_id=@SeasonId order by version desc",new{tenant.TenantId,SeasonId=seasonId},tx,cancellationToken:cancellationToken))).Select(MapVersion).ToArray();
        var lastClosed=versions.FirstOrDefault(x=>x.State=="CLOSED");
        ...
        return new SeasonClosingDto(scope,lastClosed?.State??(runRow is null?"PREPARATION":"CHECKED"),indicators,runRow is null?null:new SeasonClosingRunDto(runRow.Id,runRow.GeneratedAt,runRow.CriteriaVersion,DeserializeIssues(runRow.Issues)),versions,retroactive);
    },cancellationToken);

    public Task<SeasonClosingRunDto> RunClosingChecksAsync(RunSeasonClosingCommand command,CancellationToken cancellationToken) => database.InTenantTransactionAsync(async(db,tx)=>
    {
        ...
        if(prior is not null)return new(prior.Id,prior.GeneratedAt,prior.CriteriaVersion,DeserializeIssues(prior.Issues));
        var scope=await ClosingScope(db,tx,command.SeasonId,command.CutoffDate,cancellationToken);var indicators=await ClosingIndicators(db,tx,command.SeasonId,command.CutoffDate,scope.Unit,cancellationToken);var issues=BuildIssues(command.SeasonId,indicators);var id=Guid.CreateVersion7();
        await db.ExecuteAsync(new CommandDefinition("insert into agro360.harvest_closing_runs(id,tenant_id,season_id,farm_id,cutoff_date,criteria_version,issues,idempotency_key,created_by) values(@Id,@TenantId,@SeasonId,@FarmId,@Cutoff,'1.0',cast(@Issues as jsonb),@Key,@UserId)",new{Id=id,tenant.TenantId,command.SeasonId,scope.FarmId,Cutoff=command.CutoffDate,Issues=JsonSerializer.Serialize(issues),Key=key,UserId=tenant.UserId},tx,cancellationToken:cancellationToken));await Audit(db,tx,"check","HarvestManagementClosing",id,cancellationToken);return new(id,DateTimeOffset.UtcNow,"1.0",issues);
    },cancellationToken);

    public Task<SeasonClosingVersionDto> CreateClosingAsync(CreateSeasonClosingCommand command,CancellationToken cancellationToken)=>database.InTenantTransactionAsync(async(db,tx)=>
    {
        var key=Required(command.IdempotencyKey,nameof(command.IdempotencyKey));var prior=await db.QuerySingleOrDefaultAsync<ClosingVersionRow>(new CommandDefinition("select id,version,state,cutoff_date CutoffDate,generated_at GeneratedAt,responsible_id ResponsibleId,supersedes_id SupersedesId,reason,notes,indicators::text Indicators,issues::text Issues from agro360.harvest_closing_versions where tenant_id=@TenantId and idempotency_key=@Key",new{tenant.TenantId,Key=key},tx,cancellationToken:cancellationToken));if(prior is not null)return MapVersion(prior);
        await db.ExecuteAsync(new CommandDefinition("select pg_advisory_xact_lock(hashtextextended(@LockKey,0))",new{LockKey=$"{tenant.TenantId}:harvest-closing:{command.SeasonId}"},tx,cancellationToken:cancellationToken));var scope=await ClosingScope(db,tx,command.SeasonId,command.CutoffDate,cancellationToken);var indicators=await ClosingIndicators(db,tx,command.SeasonId,command.CutoffDate,scope.Unit,cancellationToken);var issues=BuildIssues(command.SeasonId,indicators);
    ...
    private static IReadOnlyCollection<SeasonClosingIssueDto> DeserializeIssues(string json)=>JsonSerializer.Deserialize<SeasonClosingIssueDto[]>(json,new JsonSerializerOptions{PropertyNameCaseInsensitive=true})??[];
    private static SeasonClosingVersionDto MapVersion(ClosingVersionRow x)=>new(x.Id,x.Version,x.State,x.CutoffDate,x.GeneratedAt,x.ResponsibleId,x.SupersedesId,x.Reason,x.Notes,JsonSerializer.Deserialize<SeasonClosingIndicatorDto[]>(x.Indicators,new JsonSerializerOptions{PropertyNameCaseInsensitive=true})??[],DeserializeIssues(x.Issues));
    ...
    private sealed record ClosingVersionRow(Guid Id,int Version,string State,DateOnly CutoffDate,DateTimeOffset GeneratedAt,Guid ResponsibleId,Guid? SupersedesId,string? Reason,string? Notes,string Indicators,string Issues);
