...
public sealed partial class SaasService(DatabaseExecutor db, ITenantContext tenant, IPasswordHasher passwordHasher, ILogger<SaasService> logger) : ISaasService
{
    ...
    {
        var current = await c.QuerySingleOrDefaultAsync<string>("select status from agro360.saas_organizations where tenant_id=@Id for update", new { id }, t) ?? throw new KeyNotFoundException("Organização não encontrada.");
        SaasGovernanceRules.EnsureStatusTransition(current, status, reason);
        await c.ExecuteAsync("select set_config('app.tenant_id',@TenantContext,true)", new { TenantContext = id.ToString() }, t);
        if (status is "SUSPENDED" or "BLOCKED")
            await c.ExecuteAsync("insert into agro360.saas_tenant_restrictions(tenant_id,cause,reason,created_by) select @Id,'ADMINISTRATIVE',@Reason,@Actor where not exists(select 1 from agro360.saas_tenant_restrictions where tenant_id=@Id and cause='ADMINISTRATIVE' and ended_at is null)", new { id, Reason = reason!.Trim(), Actor = actorId }, t);
        else if (status == "ACTIVE")
            await c.ExecuteAsync("update agro360.saas_tenant_restrictions set ended_at=now(),ended_by=@Actor,end_reason=@Reason where tenant_id=@Id and cause='ADMINISTRATIVE' and ended_at is null", new { id, Reason = reason!.Trim(), Actor = actorId }, t);
        var financialRestriction = await c.ExecuteScalarAsync<bool>("select exists(select 1 from agro360.saas_tenant_restrictions where tenant_id=@Id and cause='FINANCIAL' and ended_at is null)", new { id }, t);
    ...
    public Task<Guid> SaveUserAsync(Guid? id, UserCommand command, Guid actorId, CancellationToken ct) => Tenant("save-user", async (c, t) =>
    {
        ...
        if (id is null)
        {
            var limit = await c.QuerySingleAsync<(long Used, int Limit)>("select count(*) filter(where u.status='ACTIVE') Used,p.user_limit Limit from agro360.saas_organizations o join agro360.saas_plans p on p.id=o.plan_id left join agro360.identity_users u on u.tenant_id=o.tenant_id and u.deleted_at is null where o.tenant_id=@TenantId group by p.user_limit", new { tenant.TenantId }, t);
            if (limit.Used >= limit.Limit) { InfrastructureLogMessages.UserLimitExceeded(logger, tenant.TenantId); throw new ConflictException($"Limite do plano atingido: {limit.Used} de {limit.Limit} usuários.", "plan_user_limit"); }
    ...
    public Task<IReadOnlyList<BillingChargeSummary>> GetChargesAsync(CancellationToken ct) => System("billing-list", async (c, t) => (IReadOnlyList<BillingChargeSummary>)(await c.QueryAsync<BillingChargeSummary>("select c.id,c.tenant_id TenantId,x.name TenantName,p.name PlanName,c.competence,c.due_on DueOn,c.base_amount BaseAmount,c.additional_amount AdditionalAmount,c.discount,c.amount,coalesce(c.paid_amount,0) PaidAmount,greatest(c.amount-coalesce(c.paid_amount,0),0) OutstandingAmount,c.module_codes ModuleCodes,c.status,c.notes,c.paid_on PaidOn from agro360.saas_billing_charges c join agro360.tenancy_tenants x on x.id=c.tenant_id join agro360.saas_subscriptions s on s.id=c.subscription_id join agro360.saas_plans p on p.id=s.plan_id where c.deleted_at is null order by c.due_on desc", transaction: t)).ToArray(), ct);
    public Task<Guid> CreateChargeAsync(BillingChargeCommand command, Guid actorId, CancellationToken ct) => System("billing-create", async (c, t) => { var total = SaasGovernanceRules.CalculateChargeTotal(command.Amount, command.AdditionalAmount, command.Discount); if (total <= 0 || command.DueOn < command.Competence || command.Competence.Day != 1) throw new ArgumentException("Competência, vencimento e valor da cobrança são inválidos."); var id = Guid.CreateVersion7(); var n = await c.ExecuteAsync("insert into agro360.saas_billing_charges(id,tenant_id,subscription_id,competence,due_on,base_amount,additional_amount,discount,amount,module_codes,status,notes,created_by) select @Id,@TenantId,@SubscriptionId,@Competence,@DueOn,@Amount,@AdditionalAmount,@Discount,@Total,@ModuleCodes,'DRAFT',@Notes,@Actor from agro360.saas_subscriptions where id=@SubscriptionId and tenant_id=@TenantId and deleted_at is null; insert into agro360.saas_billing_charge_events(tenant_id,charge_id,event_type,reason,details,created_by) select @TenantId,@Id,'CREATED','Registro gerencial manual',jsonb_build_object('amount',@Total,'moduleCodes',@ModuleCodes),@Actor where exists(select 1 from agro360.saas_billing_charges where id=@Id)", new { Id = id, command.TenantId, command.SubscriptionId, command.Competence, command.DueOn, command.Amount, command.AdditionalAmount, command.Discount, Total = total, ModuleCodes = command.ModuleCodes ?? [], command.Notes, Actor = actorId }, t); if (n == 0) throw new KeyNotFoundException("Assinatura não encontrada para o cliente."); return id; }, ct);
    public Task ChangeChargeStatusAsync(Guid id, BillingStatusCommand command, Guid actorId, CancellationToken ct) => System("billing-status", async (c, t) => { var status = command.Status.ToUpperInvariant(); if (status is not ("ISSUED" or "PAID" or "CANCELLED" or "NEGOTIATING") || string.IsNullOrWhiteSpace(command.Reason) || command.Reason.Trim().Length < 5) throw new ArgumentException("Status e justificativa são obrigatórios."); if (status == "PAID" && (command.PaidOn is null || command.PaidAmount is null or <= 0 || string.IsNullOrWhiteSpace(command.Observation))) throw new ArgumentException("A baixa manual exige data, valor e observação."); var n = await c.ExecuteAsync("with changed as (update agro360.saas_billing_charges set status=@Status,paid_on=case when @Status='PAID' then @PaidOn else paid_on end,paid_amount=case when @Status='PAID' then @PaidAmount else paid_amount end,payment_method=case when @Status='PAID' then 'MANUAL' else payment_method end,cancellation_reason=case when @Status='CANCELLED' then @Reason else cancellation_reason end,notes=case when @Status='PAID' then concat_ws(E'\\n',notes,@Observation) else notes end,updated_at=now(),updated_by=@Actor where id=@Id and deleted_at is null and status not in('PAID','CANCELLED') returning tenant_id) insert into agro360.saas_billing_charge_events(tenant_id,charge_id,event_type,reason,details,created_by) select tenant_id,@Id,@Status,@Reason,jsonb_build_object('paidOn',@PaidOn,'paidAmount',@PaidAmount),@Actor from changed", new { id, Status = status, Reason = command.Reason.Trim(), command.PaidOn, command.PaidAmount, command.Observation, Actor = actorId }, t); if (n == 0) throw new KeyNotFoundException("Cobrança não encontrada ou já encerrada."); }, ct);
    public Task<ManualPaymentResult> RegisterManualPaymentAsync(Guid id, ManualPaymentCommand command, Guid actorId, CancellationToken ct) => System("billing-payment", async (c, t) =>
    {
        if (string.IsNullOrWhiteSpace(command.IdempotencyKey) || command.IdempotencyKey.Trim().Length > 120 || string.IsNullOrWhiteSpace(command.PaymentReference) || command.PaymentReference.Trim().Length < 3 || string.IsNullOrWhiteSpace(command.Observation) || command.Observation.Trim().Length < 5)
            throw new ArgumentException("Chave de idempotência, identificação e observação do pagamento são obrigatórias.");
        await c.ExecuteAsync("select pg_advisory_xact_lock(hashtextextended(@Key,0))", new { Key = $"saas-payment:{id:N}:{command.IdempotencyKey.Trim()}" }, t);
        var replay = await c.QuerySingleOrDefaultAsync<ManualPaymentResult>(
            "select p.id PaymentId,c.status ChargeStatus,p.applied_amount AppliedAmount,greatest(c.amount-coalesce(c.paid_amount,0),0) OutstandingAmount,p.credit_amount CreditAmount,true Replayed from agro360.saas_billing_payments p join agro360.saas_billing_charges c on c.id=p.charge_id where p.charge_id=@Id and p.idempotency_key=@Key",
            new { Id = id, Key = command.IdempotencyKey.Trim() }, t);
        if (replay is not null) return replay;
        var charge = await c.QuerySingleOrDefaultAsync<ChargePaymentLookup>("select tenant_id TenantId,amount,coalesce(paid_amount,0) PaidAmount,status from agro360.saas_billing_charges where id=@Id and deleted_at is null for update", new { id }, t)
            ?? throw new KeyNotFoundException("Cobrança não encontrada.");
        if (charge.Status is "CANCELLED" or "PAID") throw new ConflictException("Cobrança encerrada não aceita nova baixa.", "charge_closed");
        var allocation = SaasGovernanceRules.AllocatePayment(charge.Amount, charge.PaidAmount, command.Amount);
        var paymentId = Guid.CreateVersion7();
        await c.ExecuteAsync(
            """
            select set_config('app.tenant_id',@TenantContext,true);
            insert into agro360.saas_billing_payments(id,tenant_id,charge_id,idempotency_key,payment_reference,amount,applied_amount,credit_amount,paid_on,notes,created_by)
            values(@PaymentId,@TenantId,@Id,@Key,@Reference,@Amount,@Applied,@Credit,@PaidOn,@Observation,@Actor);
            update agro360.saas_billing_charges set status=@Status,paid_on=@PaidOn,paid_amount=coalesce(paid_amount,0)+@Applied,payment_method='MANUAL',notes=concat_ws(E'\n',notes,@Observation),updated_at=now(),updated_by=@Actor where id=@Id;
            insert into agro360.saas_billing_credits(id,tenant_id,payment_id,amount,created_by)
            select gen_random_uuid(),@TenantId,@PaymentId,@Credit,@Actor where @Credit>0;
            insert into agro360.saas_billing_charge_events(tenant_id,charge_id,event_type,reason,details,created_by)
            values(@TenantId,@Id,@Status,'Baixa manual conciliada',jsonb_build_object('paymentId',@PaymentId,'reference',@Reference,'received',@Amount,'applied',@Applied,'credit',@Credit),@Actor);
            """, new { PaymentId = paymentId, TenantId = charge.TenantId, TenantContext = charge.TenantId.ToString(), Id = id, Key = command.IdempotencyKey.Trim(), Reference = command.PaymentReference.Trim(), command.Amount, command.PaidOn, Observation = command.Observation.Trim(), Actor = actorId, Status = allocation.ChargeStatus, Applied = allocation.AppliedAmount, Credit = allocation.CreditAmount }, t);
        return new ManualPaymentResult(paymentId, allocation.ChargeStatus, allocation.AppliedAmount, allocation.OutstandingAmount, allocation.CreditAmount, false);
    }, ct);
    ...
    private async Task<T> Guard<T>(string op, Func<Task<T>> work) { try { return await work(); } catch (Exception ex) { InfrastructureLogMessages.SaasFailed(logger, op, tenant.IsAvailable ? tenant.TenantId : Guid.Empty, ex); throw; } }
    private async Task Guard(string op, Func<Task> work) { try { await work(); } catch (Exception ex) { InfrastructureLogMessages.SaasFailed(logger, op, tenant.IsAvailable ? tenant.TenantId : Guid.Empty, ex); throw; } }
