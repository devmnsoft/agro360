...
public sealed class SeasonTrackingService(DatabaseExecutor database, ITenantContext tenant) : ISeasonTrackingService
{
    public Task<SeasonTrackingOverview> OverviewAsync(Guid seasonId, DateOnly? referenceDate, CancellationToken ct) =>
        database.InTenantTransactionAsync<SeasonTrackingOverview>(async (c, t) =>
        {
            ...
            var history = (await c.QueryAsync(new CommandDefinition("""
                ...
                """, new { tenant.TenantId, SeasonId = seasonId }, t, cancellationToken: ct))).ToArray();
            var completed = operations.Count(command => command.Status == "COMPLETED");
            var inProgress = operations.Count(command => command.Status is "IN_PROGRESS" or "PARTIAL");
            var overdue = operations.Count(command => command.Status is not ("COMPLETED" or "CANCELLED") && DateOnly.FromDateTime(command.PlannedEnd.UtcDateTime) < cutoff);
            var executed = operations.Sum(command => command.ExecutedAreaHa); // worked area, intentionally not physical area
            ...
            var pending = operations.Where(command => command.BlockReason is not null).Select(command => new SeasonPending("PLANNING", command.BlockReason!, command.PlannedStart, command.ResponsibleId?.ToString(), "Impede a liberação da operação.", "Abrir a predecessora e regularizar ou solicitar exceção.", command.BlockUrl!, "BLOCKER")).ToList();
            pending.AddRange(operations.Where(command => command.ResponsibleId is null && command.Status != "CANCELLED").Select(command => new SeasonPending("PLANNING", "Operação sem responsável.", command.PlannedStart, null, "A ordem não pode ser liberada.", "Definir responsável no planejamento.", $"/Agriculture?operationId={command.Id}", "BLOCKER")));
            pending.AddRange(operations.Where(command => command.Status is not ("COMPLETED" or "CANCELLED") && DateOnly.FromDateTime(command.PlannedEnd.UtcDateTime) < cutoff).Select(command => new SeasonPending("SCHEDULE", "Atividade atrasada.", command.PlannedEnd, command.ResponsibleId?.ToString(), "Pode comprometer a próxima operação.", "Executar ou reprogramar com justificativa.", $"/Agriculture?operationId={command.Id}", "WARNING")));
    ...
    public Task<PlanOperationDto> AddOperationAsync(Guid planId, PlanOperationCommand command, CancellationToken ct)
    {
        ValidateOperation(command.PlannedStart, command.PlannedEnd, command.PlannedAreaHa);
        return database.InTenantTransactionAsync<PlanOperationDto>(async (c, t) =>
        {
            var scope = await ValidateScope(c, t, planId, command.SeasonId, command.FarmId, command.FieldId, ct);
            if ((command.PlannedStart.Date < scope.StartsOn.ToDateTime(TimeOnly.MinValue) || command.PlannedEnd.Date > scope.EndsOn.ToDateTime(TimeOnly.MinValue)) && string.IsNullOrWhiteSpace(command.OutsideSeasonReason))
            ...
            await c.ExecuteAsync(new CommandDefinition("""
                ...
                """, new { Id = id, tenant.TenantId, PlanId = planId, command.SeasonId, command.FarmId, command.FieldId, Name = command.Name.Trim(), OperationType = command.OperationType.Trim(), command.PlannedStart, command.PlannedEnd, command.PlannedAreaHa, command.PlannedHours, command.ResponsibleId, command.OutsideSeasonReason, tenant.UserId }, t, cancellationToken: ct));
            return Map(await GetOperation(c, t, id, ct));
        }, ct);
    }

    public Task AddDependencyAsync(Guid operationId, OperationDependencyCommand command, CancellationToken ct) => database.InTenantTransactionAsync(async (c, t) =>
    {
        var kind = command.BlockingType.Trim().ToUpperInvariant(); var condition = command.ReleaseCondition.Trim().ToUpperInvariant();
        if (operationId == command.PredecessorId) throw new DomainException("Uma atividade não pode depender dela mesma.", "agriculture.dependency_self");
        ...
        await GetOperation(c, t, operationId, ct); await GetOperation(c, t, command.PredecessorId, ct);
        var cycle = await c.ExecuteScalarAsync<bool>(new CommandDefinition("""
            ...
            """, new { tenant.TenantId, OperationId = command.PredecessorId, PredecessorId = operationId }, t, cancellationToken: ct));
        if (cycle) throw new ConflictException("A dependência criaria um ciclo no planejamento.", "agriculture.dependency_cycle");
        await c.ExecuteAsync(new CommandDefinition("insert into agro360.agriculture_operation_dependencies(id,tenant_id,operation_id,predecessor_id,blocking_type,release_condition,created_by,updated_by) values(@Id,@TenantId,@OperationId,@PredecessorId,@Kind,@Condition,@UserId,@UserId)", new { Id = Guid.CreateVersion7(), tenant.TenantId, OperationId = operationId, command.PredecessorId, Kind = kind, Condition = condition, tenant.UserId }, t, cancellationToken: ct));
    }, ct);

    public Task<Guid> GenerateOrderAsync(Guid operationId, GenerateOperationOrderCommand command, CancellationToken ct)
    {
        if (command.CoveredAreaHa <= 0 || string.IsNullOrWhiteSpace(command.IdempotencyKey)) throw new DomainException("Cobertura e chave de idempotência são obrigatórias.");
        return database.InTenantTransactionAsync<Guid>(async (c, t) =>
        {
            var replay = await c.ExecuteScalarAsync<Guid?>(new CommandDefinition("select work_order_id from agro360.agriculture_operation_orders where tenant_id=@TenantId and idempotency_key=@Key", new { tenant.TenantId, Key = command.IdempotencyKey }, t, cancellationToken: ct));
            ...
            if (covered + command.CoveredAreaHa > o.PlannedAreaHa) throw new ConflictException("A cobertura das ordens supera a área planejada.", "agriculture.order_coverage_exceeded");
            ...
            await c.ExecuteAsync(new CommandDefinition("""
                ...
                """, new { Id = id, tenant.TenantId, Number = $"OC-{DateTime.UtcNow:yyyy}-{number:000000}", o.Name, Type = o.OperationType, o.FarmId, o.FieldId, o.SeasonId, o.ResponsibleId, Starts = o.PlannedStart, Ends = o.PlannedEnd, Area = command.CoveredAreaHa, OperationId = operationId, tenant.UserId }, t, cancellationToken: ct));
            await c.ExecuteAsync(new CommandDefinition("insert into agro360.agriculture_operation_orders(tenant_id,operation_id,work_order_id,covered_area_ha,idempotency_key,created_by) values(@TenantId,@OperationId,@Id,@Area,@Key,@UserId)", new { tenant.TenantId, OperationId = operationId, Id = id, Area = command.CoveredAreaHa, Key = command.IdempotencyKey, tenant.UserId }, t, cancellationToken: ct));
            return id;
        }, ct);
    }

    public Task<PlanOperationDto> RescheduleAsync(Guid operationId, RescheduleOperationCommand command, CancellationToken ct)
    {
        ValidateOperation(command.PlannedStart, command.PlannedEnd, command.PlannedAreaHa); if (string.IsNullOrWhiteSpace(command.Reason)) throw new DomainException("Informe o motivo da reprogramação.");
        return database.InTenantTransactionAsync<PlanOperationDto>(async (c, t) =>
        {
            ...
            if (command.PlannedStart.Date < seasonWindow.StartsOn.ToDateTime(TimeOnly.MinValue) || command.PlannedEnd.Date > seasonWindow.EndsOn.ToDateTime(TimeOnly.MinValue))
            ...
            if (command.PlannedAreaHa < before.ScheduledAreaHa) throw new ConflictException("A área revisada não pode ser menor que a cobertura das ordens já vinculadas.", "agriculture.order_coverage_exceeded");
            ...
            await c.ExecuteAsync(new CommandDefinition("insert into agro360.agriculture_plan_revisions(id,tenant_id,operation_id,version,reason,before_value,after_value,impact,created_by) values(@Id,@TenantId,@OperationId,@Version,@Reason,cast(@Before as jsonb),cast(@After as jsonb),jsonb_build_object('linkedOrders',@LinkedOrders,'releasedOrdersNotChanged',@Released),@UserId)", new { Id = Guid.CreateVersion7(), tenant.TenantId, OperationId = operationId, Version = after.Version, Reason = command.Reason.Trim(), Before = JsonSerializer.Serialize(before), After = JsonSerializer.Serialize(after), LinkedOrders = await c.ExecuteScalarAsync<int>(new CommandDefinition("select count(*) from agro360.agriculture_operation_orders where tenant_id=@TenantId and operation_id=@OperationId", new { tenant.TenantId, OperationId = operationId }, t, cancellationToken: ct)), Released = released, tenant.UserId }, t, cancellationToken: ct));
            return Map(after);
        }, ct);
    }

    private const string OperationSql = """select o.*,coalesce((select sum(l.physical_area_ha) from agro360.agriculture_operation_orders command join agro360.field_work_logs l on l.tenant_id=command.tenant_id and l.work_order_id=command.work_order_id and l.deleted_at is null where command.tenant_id=o.tenant_id and command.operation_id=o.id),0) ExecutedAreaHa,coalesce((select sum(command.covered_area_ha) from agro360.agriculture_operation_orders command where command.tenant_id=o.tenant_id and command.operation_id=o.id),0) ScheduledAreaHa,(select 'Predecessora '||p.name||' está '||p.status from agro360.agriculture_operation_dependencies d join agro360.agriculture_plan_operations p on p.tenant_id=d.tenant_id and p.id=d.predecessor_id where d.tenant_id=o.tenant_id and d.operation_id=o.id and d.blocking_type='REQUIRED' and d.deleted_at is null and d.exception_authorized_at is null and p.status<>'COMPLETED' order by p.planned_start limit 1) BlockReason,(select '/Agriculture?operationId='||p.id from agro360.agriculture_operation_dependencies d join agro360.agriculture_plan_operations p on p.tenant_id=d.tenant_id and p.id=d.predecessor_id where d.tenant_id=o.tenant_id and d.operation_id=o.id and d.blocking_type='REQUIRED' and d.deleted_at is null and d.exception_authorized_at is null and p.status<>'COMPLETED' order by p.planned_start limit 1) BlockUrl from agro360.agriculture_plan_operations o""";
    ...
    private static PlanOperationDto Map(OperationRow command) => new(command.Id,command.PlanRecordId,command.SeasonId,command.FarmId,command.FieldId,command.Name,command.OperationType,command.PlannedStart,command.PlannedEnd,command.PlannedAreaHa,command.PlannedHours,command.ResponsibleId,command.Status,command.Version,command.ExecutedAreaHa,command.ScheduledAreaHa,Math.Max(0,command.PlannedAreaHa-command.ScheduledAreaHa),command.BlockReason,command.BlockUrl);
