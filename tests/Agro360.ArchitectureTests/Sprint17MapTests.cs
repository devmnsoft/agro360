...
public sealed class Sprint17MapTests
{
    ...
    [Fact] public void FullSqlIsPostgresqlOnlyAndMultitenant() { var sql = File.ReadAllText(Path.Combine(Root, "database/agro360-postgres-full.sql")); Assert.Contains("create schema if not exists agro360", sql, StringComparison.OrdinalIgnoreCase); Assert.Contains("enable row level security", sql); Assert.Contains("geojson jsonb", sql); Assert.DoesNotContain("postgis", sql, StringComparison.OrdinalIgnoreCase); Assert.DoesNotContain("\\i", sql); }
