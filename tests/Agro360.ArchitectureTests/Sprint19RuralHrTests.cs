...
public sealed class Sprint19RuralHrTests
{
 ...
 [Fact]public void DatabaseIsStandaloneAndMultitenant(){var s=File.ReadAllText(Path.Combine(Root,"database/agro360-postgres-full.sql"));Assert.Contains("create schema if not exists agro360",s);Assert.Contains("unique(tenant_id,document)",s);Assert.Contains("force row level security",s);Assert.DoesNotContain("\\i",s);}
}
