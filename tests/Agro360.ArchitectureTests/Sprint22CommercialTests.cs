...
public sealed class Sprint22CommercialTests
{
 ...
 [Fact]public void Full_sql_contains_commercial_schema(){var sql=File.ReadAllText(Path.Combine(Root,"database/agro360-postgres-full.sql"));Assert.Contains("create table if not exists crm.customers",sql);Assert.Contains("create table if not exists sales.commissions",sql);Assert.Contains("create table if not exists sales.split_participants",sql);Assert.DoesNotContain("\\i ",sql);}
