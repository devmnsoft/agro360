...
public sealed class Sprint22CommercialTests
{
    ...
    [Fact] public void OrderPolicyComesFromServerAndTransitionsAreGuarded() { var contract = File.ReadAllText(Path.Combine(Root, "src/Modules/Agro360.Application/Contracts/Commercial360Contracts.cs")); var service = File.ReadAllText(Path.Combine(Root, "src/Modules/Agro360.Infrastructure/Services/Commercial360Service.cs")); Assert.DoesNotContain("MaximumDiscount", contract); Assert.Contains("sales_price_table_items", service); Assert.Contains("for update", service); Assert.Contains("ValidateOrderTransition", service); }
}
