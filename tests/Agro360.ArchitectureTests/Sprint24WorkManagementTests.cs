...
public sealed class Sprint24WorkManagementTests
{
    ...
    [Fact] public void OperationCenterDerivesPendingItemsAndNeverAllowsManualResolution() { var service = File.ReadAllText(Path.Combine(Root(), "src/Modules/Agro360.Infrastructure/Services/WorkManagementService.cs")); Assert.Contains("OperationCenterAsync", service); Assert.Contains("user_permissions", service); Assert.Contains("select count(*) from filtered", service); Assert.Contains("operation_occurrence_reads", service); Assert.DoesNotContain("ResolveOccurrenceAsync", service); }
    [Fact] public void OperationCenterInteractionIsTenantScopedAndAudited() { var sql = File.ReadAllText(Path.Combine(Root(), "database/migrations/074_operation_center_reliability.sql")); Assert.Contains("primary key(tenant_id,occurrence_key,user_id)", sql); Assert.Contains("operation_occurrence_events", sql); Assert.Contains("platform_enable_tenant_rls", sql); }
