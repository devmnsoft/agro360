...
public sealed class Sprint27PortalTests
{
    ...
    [Fact] public void PortalIsSeparateAndHasRealActions() { var js = File.ReadAllText(Path.Combine(Root, "src/Hosts/Agro360.Web/wwwroot/js/portal.js")); foreach (var route in new[] { "/api/portal/access/login", "/api/portal/dashboard", "/api/portal/marketplace", "/api/portal/requests" }) Assert.Contains(route, js); }
