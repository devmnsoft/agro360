...
public sealed class Sprint47CrmTests
{
 ...
 [Fact] public void Full_database_contains_tenant_isolated_crm(){var sql=File.ReadAllText(Path.Combine(Root,"database/agro360-postgres-full.sql"));Assert.Contains("commercial.crm_leads",sql);Assert.Contains("enable row level security",sql);Assert.Contains("commercial.commercial_contracts",sql);Assert.Contains("support.customer_success_health_scores",sql);}
